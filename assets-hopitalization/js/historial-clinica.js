 ...
 $('#' + $('#id_hab_tx').val() + 'hab_t_drug').on('change', function() {
      ...
      if (txtinput == 0 && txtarea == 0) {
   	...
   	$(document).on('click', '#paginate-examsistema-li li', function() {
			...
			var controller = "hosp_examen_sitema";
			var pageNum = this.id;
...
$(document).on('click', "#resetExamFis", function(event) {
  	event.preventDefault();
   		...
   		var controller = "hosp_exam_fisico";
...
function saveExamenFisico(id_sig) {
 ...
 var hallazgo= $('#'+id_sig+ '_hallazgo_exf').val();
     ...
     var signo_fcf = $('#' + id_sig + '_signo_fcf').val();
      ...
      $.ajax({
        type: "POST",
		   url: base_url+"hosp_exam_fisico/saveExamenFisico",
       ...
       inserted_by: $('#ex_vitales_in_by').val(),
        ...
        success: function(data) {
          ...
          loadPagination('hosp_exam_fisico'); 
...
$(document).on('click', '#saveEditExamSist', function(event) {
	 ...
	 function saveExamenSistema(id_exam_sist) {
      ...
      $.ajax({
        type: "POST", 
        url: base_url+"hosp_examen_sitema/saveExamenSistema",
        data: {
          sisneuro: sisneuro,
	  ...
	  $('#saveEditExamSist').prop("disabled", false);
          ...
          loadPagination('hosp_examen_sitema'); 
	...
	//===SAVE EXAMEN NEURO============================
   $(document).on('click', '#saveEditExamNeuro', function(e) {
	e.preventDefault(); 
saveExamNeuro($('#id_examneuro').val());

})

function saveExamNeuro(id){
   
 var exam_gen_neuro=$("#"+ id + "_exam_gen_neuro").val();

var olfatorio = $('input:radio[name='+ id +'_olfatorio]:checked').val();
var optico= $('input:radio[name='+ id +'_optico]:checked').val();
var motor_ocr_com = $('input:radio[name='+ id +'_motor_ocr_com]:checked').val();
 
 var patetica = $('input:radio[name='+ id +'_patetica]:checked').val();
 
 
 var trigemino = $('input:radio[name='+ id +'_trigemino]:checked').val();


var motor_ocu_ext = $('input:radio[name='+ id +'_motor_ocu_ext]:checked').val();;

 var facial = $('input:radio[name='+ id +'_facial]:checked').val();
 
 var estatoacustico = $('input:radio[name='+ id +'_estatoacustico]:checked').val();
 
 var glosofaringeo = $('input:radio[name='+ id +'_glosofaringeo]:checked').val();
 
 var neumogastrico = $('input:radio[name='+ id +'_neumogastrico]:checked').val();
 
 var espinal = $('input:radio[name='+ id +'_espinal]:checked').val();
 
 var hipo_mayor = $('input:radio[name='+ id +'_hipo_mayor]:checked').val();
 
 var sistema_motor = $('input:radio[name='+ id +'_sistema_motor]:checked').val();

 var marcha=$("#"+ id + "_marcha").val();
 
 

 
  var espontanea= [];
 $('input[name=' + id + '_espontanea]:checked').each(function(){
            espontanea.push(this.value);
 });
 
  var a_la_orden_verbal= [];
 $('input[name=' + id + '_a_la_orden_verbal]:checked').each(function(){
            a_la_orden_verbal.push(this.value);
 });

 var a_estimulo_doloroso= [];
 $('input[name=' + id + '_a_estimulo_doloroso]:checked').each(function(){
            a_estimulo_doloroso.push(this.value);
 });
 
 
  var no_ay_respuesta= [];
 $('input[name=' + id + '_no_ay_respuesta]:checked').each(function(){
            no_ay_respuesta.push(this.value);
 });
 
  var orientada= [];
 $('input[name=' + id + '_orientada]:checked').each(function(){
            orientada.push(this.value);
 });
 
 
   var confusa= [];
 $('input[name=' + id + '_confusa]:checked').each(function(){
            confusa.push(this.value);
 });
 
  
var inapropriada= [];
 $('input[name=' + id + '_inapropriada]:checked').each(function(){
            inapropriada.push(this.value);
 });
 
 
 var incomprensible= [];
 $('input[name=' + id + '_incomprensible]:checked').each(function(){
            incomprensible.push(this.value);
 });
 
 
  var a_la_orden_verbal_6= [];
 $('input[name=' + id + '_a_la_orden_verbal_6]:checked').each(function(){
            a_la_orden_verbal_6.push(this.value);
 });
 
 
   var localizar_dolor= [];
 $('input[name=' + id + '_localizar_dolor]:checked').each(function(){
            localizar_dolor.push(this.value);
 });
 
    var retiro_ante_el_dolor= [];
 $('input[name=' + id + '_retiro_ante_el_dolor]:checked').each(function(){
            retiro_ante_el_dolor.push(this.value);
 });
 
 
     var flexion_normal= [];
 $('input[name=' + id + '_flexion_normal]:checked').each(function(){
            flexion_normal.push(this.value);
 });
 
 
     var extension= [];
 $('input[name=' + id + '_extension]:checked').each(function(){
            extension.push(this.value);
 });
 
 
      var no_hay_respuesta= [];
 $('input[name=' + id + '_no_hay_respuesta]:checked').each(function(){
            no_hay_respuesta.push(this.value);
 });
 

var bicipital=$("#"+ id + "_bicipital").val();
var tricipital=$("#"+ id + "_tricipital").val();
var aquileo_y_clonus=$("#"+ id + "_aquileo_y_clonus").val();
var patelar_y_clonus=$("#"+ id + "_patelar_y_clonus").val();
var dedo_dedo=$("#"+ id + "_dedo_dedo").val(); 
var dedo_nariz=$("#"+ id + "_dedo_nariz").val();
var talon_rod=$("#"+ id + "_talon_rod").val();
var romberg=$("#"+ id + "_romberg").val(); 
var sensibilidad1=$("#"+ id + "_sensibilidad1").val();  
var sensibilidad2=$("#"+ id + "_sensibilidad2").val();  
var fondo_de_ojo=$("#"+ id + "_fondo_de_ojo").val();	

 var text = $("#examn-form-inputs").val();
      var checkbox = $("#examn-form-checkbox").val();
      var radio = $("#examn-form-radio").val();


$('.saveEditExamNeuro').prop('disabled',true);
$.ajax({
  url: base_url+"hosp_exam_neuro/saveUpdateExamNeuro",
type:"post",
...
dedo_dedo:dedo_dedo,dedo_nariz:dedo_nariz,talon_rod:talon_rod,romberg:romberg,sensibilidad1:sensibilidad1,sensibilidad2:sensibilidad2,fondo_de_ojo:fondo_de_ojo,id:id
},
success: function(data){
  ...
  if(data==0){
	  ...
	  $('#saveEditExamNeuro').prop("disabled", false);
		  }else{	
	
$('.saveEditExamNeuro').prop('disabled',false);

  showalert(data, "alert-success", "alert_placeholder_exam_neuro");
  	    if(id==0){
			 
       loadPagination('hosp_exam_neuro'); 
...
$.ajax({
url:base_url+'hosp_conEgreso/saveUpConEgreso',
type:"POST",
...
if(condicion_general =="" && estado_conciencia =="")
{
  Swal.fire({
	  ...
	  $('#saveEditEvaCond').prop('disabled',false);
		 ...
		 $.ajax({
            type: 'POST',
            url:base_url+'hosp_eval_con/saveEvaluationCondition',
            data: {
