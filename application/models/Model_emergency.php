...
$query_drugs = $this->hospitalization_emgergency->query("SELECT * FROM emerg_orden_medica_recetas WHERE  historial_id=$this->ID_PATIENT AND id_register=$id AND  canceled=0");
    
$num_drugs =  ' <span class="badge bg-warning text-black badge-number number_alergy" > '.$query_drugs->num_rows().' registro(s)</span> ';

$query_est = $this->hospitalization_emgergency->query("SELECT * FROM emerg_orden_medica_estudios WHERE historial_id=$this->ID_PATIENT AND id_register=$id AND  canceled=0");
$num_est = ' <span class="badge bg-warning text-black badge-number number_alergy" > '.$query_est->num_rows().' registro(s)</span> ';

$query_lab = $this->hospitalization_emgergency->query("SELECT * FROM emerg_orden_medcia_lab WHERE historial_id=$this->ID_PATIENT AND id_register=$id AND  canceled=0");
$num_lab = ' <span class="badge bg-warning text-black badge-number number_alergy" > '.$query_lab->num_rows().' registro(s)</span> ';

$query_mg = $this->hospitalization_emgergency->query("SELECT * FROM emerg_ord_med_gen WHERE id_patient=$this->ID_PATIENT AND id_register=$id  AND  canceled=0");
