...
SetJustifyMode(5);
     ...
     // GetSigImageB64(SigImageCallback);
		...
		var id_imp="<?=$id_imp?>-<?=$who?>";
		$.ajax({
		url:'<?php echo base_url();?>emergency/saveImage',
		method:"POST",
...
{
	document.sigImg.src = "data:image/png;base64," + str;
...
<?php
if($who==1){
	$h3="Firma autorizada y sello del reclamente";
	$h2="IMPRESION DE FACTURA";
}else if($who==0){
$h3="Nombre y cedula del afiliado o familiar responsable";
$h2="IMPRESION DE FACTURA";	
}
 ?>
<h2><?=$h2?></h2>
...
<p>
<button id="SignBtn" name="SignBtn" type="button" class="btn btn-primary btn-xs" onclick="javascript:onSign()">
Actiar Cuadro De Firmar
</button>

<button id="button1" name="ClearBtn"  type="button" class="btn btn-danger btn-xs" onclick="javascript:onClear()">
Limpiar
</button>


<button  id="button2" name="DoneBtn" type="button" class="btn btn-primary btn-xs" onclick="javascript:onDone()">
Hecho
</button>
<span id="volver-impresion" style="display:none"><a  href="<?php echo base_url("admin_medico/print_billing/$id_imp/$who"); ?>"><i>Volver a la impresion</i></a></span>
