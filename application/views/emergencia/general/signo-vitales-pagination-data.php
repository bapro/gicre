...
<div class="col-md-12" >
<div id='reload-table-ex-fis'></div>
</div>
...
<div class="col-sm-6">
T. A.(mm) <input id="taex" value="<?=$rowExF->ta?>" class="form-control">
T. A. (hg) <input  id="hgex" value="<?=$rowExF->hg?>"  class="form-control">
</div>
</td>
...
success:function(data){
	...
	//$("#reload-table-ex-fis").load(" #reload-table-ex-fis > *");
	loadSignoExF();
...
loadSignoExF();
function loadSignoExF()
{
$.ajax({
url:"<?php echo base_url(); ?>hospitalizacion/loadSigno",
data: {id_exam_fis:<?=$rowExF->id?>,historial_id:<?=$id_patient?>,table:"emergency_signo_vitales"},
method:"POST",
success:function(data){
$('#reload-table-ex-fis').html(data);
}
});
}			
</script>