...
$this->db->where($where);
$this->db->delete('emergency_peticion');

    $sql = "select id, name from  emergency_medicaments where centro=$centro_id && sustenacia=''";
...
<div  class="col-md-12" style="top:20px;height:200px;overflow-y:auto;background:white; box-shadow: 
    0 0 0 1px hsl(0, 0%, 50%),
    0 0 0 2px hsl(0, 0%, 60%),
    0 0 0 3px hsl(0, 0%, 70%);"  >
	<br/>
...
data: {user_id:<?=$user_id?>,id_historial:<?=$id_historial?>,saved:1,centro:<?=$centro_id?>,showinsumoedit:''},
method:"POST",
...
$.ajax({
url:"<?php echo base_url(); ?>emergency/loadInsumo",
data: {user_id:<?=$user_id?>,id_historial:<?=$id_historial?>,saved:0,centro:<?=$centro_id?>,showinsumoedit:'none'},
method:"POST",
success:function(data){
...
data: {user_id:<?=$user_id?>,id_historial:<?=$id_historial?>,centro:<?=$centro_id?>},
method:"POST",
success:function(data){
	loadInsumo();
loadInsumoSaved();
...
data: {insumo:$('#emer-nursing1').val(),cantidad_insumo:$('#cantidad_insumo').val(),user_id:<?=$user_id?>,id_historial:<?=$id_historial?>,centro:<?=$centro_id?>,idemp:<?=$id_emergency?>},
method:"POST",
success:function(data){
