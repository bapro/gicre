...
data: {printing:2,centro:<?=$centro_id?>,medicamento_ord_med:$("select[name=medicamento_ord_med]").val(),dosis:$("input[name=dosis]").val(),
presentacion_ord_med:$("select[name=presentacion_ord_med]").val(),frecuencia_ord_med:$("select[name=frecuencia_ord_med]").val(),
via_ord_med:$("select[name=via_ord_med]").val(),oyo_ord_med:$("select[name=oyo_ord_med]").val(),operator:<?=$user_id?>,cobert:0.8,id_em:<?=$id_emergency?>,
...
$.ajax({
url:"<?php echo base_url(); ?>admin_medico/allRecetasOrdMed",
