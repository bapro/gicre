                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-md-8">
                            <div class="input-group mb-3">
                                ...
                                <input type="date" class="form-control" placeholder="" id="desde-hosp">
                                <span class="input-group-text">Hasta</span>
                                <input type="date" class="form-control" placeholder="" id="hasta-hosp">
                                <button class="btn btn-primary disable-search-button" type="button" disabled><i
...
$('.form-select').select2({
    ...
    function filter_data(page,desde,hasta)
    {
        ...
        $.ajax({
            url:"<?php echo base_url(); ?>patient_hospitalization_controller_test/fetch_patients_hoptalization_data/"+page,
            method:"POST",
            dataType:"JSON",
            data:{action:action,centro:centro, patient:patient, origine:<?=$origine?>, desde:desde, hasta:hasta},
...
function loadPatients(desde, hasta){

 $.ajax({
            url:"<?php echo base_url(); ?>patient_hospitalization_controller_test/fetch_patients/",
            method:"POST",
            dataType:"JSON",
            data:{centro:$('.centro').val(), origine:<?=$origine?>, desde:desde, hasta:hasta},
            success:function(data)
            {
				$('.patient').prop('disabled', false);
				$('.disable-search-button').prop('disabled', false);
    ...
    $(document).on('click', '.pagination li a', function(event){
        ...
        filter_data(page,$('#desde-hosp').val(), $('#hasta-hosp').val());
    });
    ...
    $('.common_selector').change(function(){
		$('.filter_data').html('');
		$('.filter_data_spin').html('<div class="spinner-border remove-this-spinner"  ></div>');
        filter_data(1,$('#desde-hosp').val(), $('#hasta-hosp').val());
        loadPatients($('#desde-hosp').val(), $('#hasta-hosp').val());
  ...
  $('.disable-search-button').click(function(){
  	alert($('#desde-hosp').val());
  	loadPatients($('#desde-hosp').val(), $('#hasta-hosp').val());
  	filter_data(1,$('#desde-hosp').val(), $('#hasta-hosp').val());
		    });
