...
<?php
$time=date("d-m-Y H:i:s", strtotime($row->updated_time)); 
$doc=$this->db->select('name,area')->where('id_user',$row->operator)->get('users')->row_array();
$name=$doc['name'];
...
$link='href="'.base_url().'estudios/viewPatientDetail/'.$row->id_i_e.'/'.$row->historial_id.'/'.$row->despachada.'/'.$id_usr.'"';
$data="
<table class='table' >
<tr>
