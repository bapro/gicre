...
$search_egreso_date = date("Y-m-d", strtotime($inserted_time)); 
$update_time = date("Y-m-d H:i:s");
...
$idpceg=$row->id_patient;
$params2 = array('table' => 'emerg_conclusion_ingreso', 'id' => $row->id);
...
<textarea class="form-control CongEgreso-txtarea resumen_hallasgos_data" placeholder="Resumen de los hallazgos" id="<?= $id_coneg ?>_resumen_hallasgos" style="height:150px"><?= $resumen_hallasgos ?></textarea>
...
<label for="<?= $id_coneg ?>_resumen_intervenciones">Resumen de la intervenciones </label>
</div>
...
<label for="<?= $id_coneg ?>_diag_alta_diag1">Diagnóstico(s) <span class="text-danger fs-5" >*</span></label>
...
<hr/>
  ...
  if($con_eg_data > 0){
	...
	$id_orm=  $this->hospitalization_emgergency->select('id')->like('inserted_time', $search_egreso_date)->where('id_historial', $idpceg)->get('emerg_orden_medica')->row('id');
	$id_sig = $this->hospitalization_emgergency->select('id')->where('id_patient', $idpceg)->order_by('id','desc')->get('emerg_exam_fisico')->row('id');
echo'<a class="btn btn-md btn-primary m-1" href="'.base_url().'print_page_emergency/conclusion_egreso/'.$id_coneg.'/'.$id_orm.'/'.$id_sig.'" target="_blank"><i class="fa fa-print"></i></a>';
  }
?>
