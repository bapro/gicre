...
//$print='<a class="btn btn-md btn-primary show-print-orden-medica" target="_blank" style="display:none" href="'.base_url().'print_page_emergency/medical_order/'.$this->session->userdata('user_id').'/'.$this->session->userdata('id_patient').'/0/" target="_blank"><i class="fa fa-print"></i> Imprimir</a>';
$print=''; 
}else{
	...
	$print='<button class="btn btn-md btn-primary" id="new-orden-medica">Nuevo</button>';
    //$print='<button class="btn btn-md btn-primary" id="new-orden-medica">Nuevo</button> <a class="btn btn-md btn-primary" target="_blank" href="'.base_url().'print_page_emergency/medical_order/'.$id_doc.'/'.$id_patient.'/'.$id_sala.'/" target="_blank"><i class="fa fa-print"></i> Imprimir</a>';
    $params = array('table' => 'emerg_orden_medica', 'id'=>$id_sala);
  ...
  <div class="p-1"> <input type="time" class="form-control" id="hospOrdenMedicaHora"   /></div>
</div>
         ...
         <div class="accordion-body">
           ...
           <div class="row">
			 ...
			 <div class="col-md-6">
				...
				$query= $this->db->query("SELECT sub_groupo  FROM centros_tarifarios_test WHERE groupo LIKE 'LABORATORIO' AND centro_id=$ID_CENTRO  GROUP BY sub_groupo");
  ...
  <?php
  echo"
$print <button type='button' id='printMedicalOrder'  class='btn btn-md btn-primary show-print-orden-medica' ><i class='fa fa-print'></i> Imprimir</button>
";?>
	</div>
     </div>
    </div>
   ...
   document.getElementById("hospOrdenMedicaHora").defaultValue = "<?=$hour_ingreso?>";
   </script>