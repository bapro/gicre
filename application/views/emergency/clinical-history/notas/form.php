...
if ($ex_notas_fis_data == 0) {
    ...
    $idExf = $row->idSearch;
    $params2 = array('table' => 'emerg_exam_fis_nota', 'id' => $row->idSearch);
...
<div id="examfisico-form-notas">
    <div class="row" id="exam-HospExf-form-notas">
                ...
                <div class="accordion-item ">
                    ...
                    <div id="collapseTwelveHospExfNotas" class="accordion-collapse collapse"
                        ...
                        <div class="accordion-body">
                            <div class="col-md-12">
                                <?php $this->load->view('emergency/clinical-history/notas/vital-signals'); ?>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="float-end text-end">
            <?php if ($idExf > 0) {
 echo '<a class="btn btn-md btn-primary" target="_blank" href="'.base_url().'print_page_emergency/print_notas/'.$idExf.'" target="_blank"><i class="fa fa-print"></i></a>';
			?>
                <button type="button" class="btn btn-primary" id="resetFormFormExFNotas">Nuevo</button>
            <?php
           } 	if($this->session->userdata('user_id')==$userExf){
			   
			    if($this->session->userdata('user_perfil') =='Enfermera' || $this->session->userdata('user_perfil') =='Medico' ){
			   ?>
