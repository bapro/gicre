...
$updatet = date("d-m-Y H:i:s", strtotime($row->update_date));
 ...
 $paciente=$this->db->select('nombre,cedula,afiliado,email,tel_cel,photo,id_p_a,photo,ced1,ced2,ced3')->where('id_p_a',$row->paciente)->get('patients_appointments')->row_array();
...
->get('users')->row_array();
$area=$this->db->select('title_area')->where('id_ar',$doctor['id_user'])->get('areas')->row('title_area');
...
<div class="col-md-2">
<a href="<?php echo base_url("$controller/billing_medicos")?>" class="btn btn-primary btn-xs st"  title="Nueva Factura"   ><i class="fa fa-plus" aria-hidden="true"></i>Nueva Factura</a>
</div>
</div>
...
$('#addr' + numRows).html("<td><input type='checkbox' name='chkbox' class='remove'></td><td <td style='width:180px;display:block'><select class='service form-control' onChange='getSegName(this);'><option value='' hidden></option><?php foreach($edit_tarifario_centro as $s){?><option  value='<?=$s->id_c_taf?>'><?=$s->sub_groupo?></option><?php }?></select></td><td class='cantidad'><input name='cantidad" + numRows + "' type='text' class='cantidad form-control'  tabindex='" + (ti++) + "' onkeypress='return validateQty(event);' /></td><td class='precio'><div class='input-group'><span class='input-group-addon'>RD$</span><input  name='precio" + numRows + "' type='text' class='precio form-control float'  tabindex='" + (ti++) + "' onkeypress='return onlyfloat(event);' /></div></td><td class='row-total'><div class='input-group'><span class='input-group-addon'>RD$</span><input type='text' class='row-total form-control' value='0.00' readonly /></div></td><td class='total-pag-seg'><div class='input-group'><span class='input-group-addon'>RD$</span><input  name='totalpagseg" + numRows + "' type='text' class='total-pag-seg form-control'  tabindex='" + (ti++) + "' readonly /></div></td><td class='descuento'><div class='input-group'><span class='input-group-addon'>RD$</span><input style='color:red' name='descuento" + numRows + "' type='text' class='descuento form-control float'  tabindex='" + (ti++) + "'onkeypress='return onlyfloat(event);' /></div></td><td class='total-pag-pa'><div class='input-group'><span class='input-group-addon'>RD$</span><input  name='totalpagpa" + numRows + "' type='text' class='total-pag-pa form-control ' value='0.00' tabindex='" + (ti++) + "' readonly /></div></td>");
...
$.ajax({
type: "POST",
url: "<?=base_url('admin_medico/updateBill')?>",
...
numauto:numauto,autopor:autopor,comment:comment,inserted_by:inserted_by,is_admin:is_admin},
cache: true,
success:function(data){ 
