...
<th class="heading">
<?php if(empty($serv)) {
	echo "<span style='color:#B18904'>No hay servicios | <a target='_blank' href='".base_url()."/medico/import_rates_fac/$id_doctor/$id_seg'>Crear</a></span>";
...
<div class="col-sm-3">
<?php
$id=$this->db->select('idfacc')->order_by('idfacc','desc')->limit(1)->get('factura2')->row('idfacc');
$last_id_fac= $id + 1;
?>
...
<i class="fa fa-check-circle" style="color:green">  Creada con con éxito !</i>
<a  target="_blank" class="btn btn-primary btn-sm" href="<?php echo base_url("medico/billing_print_view/$last_id_fac/medico")?>">Ver</a>
</div>
</div>
</div>
</div>
