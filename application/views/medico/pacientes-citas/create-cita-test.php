...
if($is_admin=="yes"){
 ...
 ?>
 <form  class="form-horizontal" enctype="multipart/form-data" id="sendcita"  method="post"   > 


 <input  type="hidden" name="creadted_by"   value="<?=$name?>"  >
<input  type="hidden" name="id_user"   value="<?=$id_user?>"  >
<button type="submit" id="sendc" style="float:right" class="btn btn-primary btn-lg enable-send  check-age" ><i class="fa fa-floppy-o" aria-hidden="true"></i>  Guardar </button>
...
<input type="text" class="form-control required" data-name="display_age"  onchange="display_age()" id="date_nacer" name="date_nacer" readonly>
...
<?php } else {
	$area= $this->db->select('area')->where('first_name',$id_user)->get('doctors')->row('area');
	echo"<input name='especialidad' type='hidden' value='$area'/>";
	echo"<input name='doctor' type='hidden' value='$id_user'/>";
	}
	?>
<div class="form-group">
...
if($("#age").val() == ""){
alert("Cual es tu fecha de nacimiento ?");
return false;
}	
})
...
$('#sendcita').on('submit', function(event){
event.preventDefault();

$.ajax({
url:"<?php echo base_url(); ?>admin_medico/save_patients_appointments",
method:"POST",
