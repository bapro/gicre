...
$id_p_a_h = encrypt_url($ver->id_patient);
$id_user_h = encrypt_url($iduser);
$id_cm_h = encrypt_url($ver->centro);
$from_view = encrypt_url(0);
$doctor_h = encrypt_url($ver->doctor);
$areaid= $this->db->select('area')->where('id_user',$ver->doctor)->get('users')->row('area');
$areaid_ad = encrypt_url($areaid);
...
$sql_con ="SELECT historial_id FROM h_c_enfermedad where filter_date='$today' AND historial_id=$ver->id_patient AND user_id=$ver->doctor AND centro_medico=$ver->centro AND enf_motivo !='' AND signopsis !='' ";
...
<?php 
   ...
   ->get('users')->row('name');
   if($perfil=="Admin"){?> <a title="<?=$area_d?>" href="<?php echo site_url("admin/doctor/".$ver->doctor); ?>"><?=$doctor;?></a> <?php }else{echo $doctor;}?>
   
   </td>
<td style="text-transform: uppercase">
<?php if($perfil=="Admin") {
	?>
<a href="<?php echo site_url("$controller/historial_medical/$id_p_a_h/$id_user_h/$areaid_ad/$id_cm_h/$hist_h/$id_user_h/$from_view"); ?>"><i  class="fa fa-history" aria-hidden="true" title='ir a la historia clinica' ></i> <?=$atend?> <em  title='<?=$frecuencia_text;?>'> <?=$frecuencia_nbr;?></em></a>
<?php } else if($perfil=="Medico"){
?>
<a href="<?php echo site_url("$controller/historial_medical/$id_p_a_h/$id_user_h/$areaid_ad/$id_cm_h/$hist_h/$id_user_h/$from_view"); ?>"><i  class="fa fa-history" aria-hidden="true" title='ir a la historia clinica' ></i> <?=$atend?> <em  title='<?=$frecuencia_text;?>'> <?=$frecuencia_nbr;?></em></a>
<?php	
}else if($perfil=="Asistente Medico" && $centro['type'] !="publico"){

$hist_ha=encrypt_url(4);
?>
<a href="<?php echo site_url("$controller/historial_medical_past/$id_p_a_h/$doctor_h/$areaid_ad/$id_cm_h/$hist_ha/$id_user_h/$from_view"); ?>"><i class="fa fa-history" aria-hidden="true" title='ir a la historia clinica'></i> <?=$atend?> <em  title='<?=$frecuencia_text;?>'> <?=$frecuencia_nbr;?></em></a>
<?php
}else{
