...
$cita= (object)$citas;
	$ci=$this->db->select('nec,fecha_propuesta')->where('id_patient',$citas->id_p_a )
   ->get('rendez_vous')->row_array();?>

<tbody>
<tr>
<td class="solicitud"><?=$ci['nec'];?></td>
<td class="nombre" ><?=$citas->nombre;?>  </td>
<td class="tel"><?=$citas->tel_resi;?></td>
<td class="telc"><?=$citas->tel_cel;?></td>
<td class="email"><?=$citas->email;?></td>
<td class="fecha" style="text-align:left"><?=$ci['fecha_propuesta'];?></td>

<td >
<div class="btn-group">
<a  class="btn btn-primary btn-xs" data-toggle="modal"   data-target="#verSolicitud" href="<?php echo site_url("cauter/request/".$citas->id_p_a)?>" >Ver</a>
