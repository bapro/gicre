			...
			<div class="card-body">
				...
				<div class="table-responsive">
					...
					<table class="table table-sm table-striped mb-0" id="table-tarifario" style="width:100%">
						...
						<tr>
						<td>
							<select class="form-select select2 form-select-sm center_tariff_area" id="center_tariff_area" onChange="center_area_on_change(this);">
						...
						<tbody>
							...
							<tr id="addr1" class="align-middle calculation">
								<!--<td class="bind-area" style="width:280px;">
								
									<span   class="tariff-area">
								</td>-->
...
<script>
	...
	function addRow() {
		...
		$('#addr' + numRows).html("<td style='width:230px;display:block'><select style='width:100%' class='form-select form-select-sm  save-servicios   select2 get-tarif-data-clone' onChange='getTarifDataSeguroPublico(this);' ></select></td><td class='cantidad'><input value='1' name='cantidad" + numRows + "' type='text' class='cantidad form-control form-control-sm'  tabindex='" + (ti++) + "' onkeypress='return validateQty(event);' /></td><td class='precio'><div class='input-group input-group-sm'><span class='input-group-text change-span-device'>RD$</span><input  name='precio" + numRows + "' type='text' class='precio form-control float'  tabindex='" + (ti++) + "' onkeypress='return onlyfloat(event);' /></div></td><td><div class='input-group input-group-sm'><span class='input-group-text change-span-device'>RD$</span><input type='text' class='row-total form-control' value='0.00' readonly /></div></td><td class='total-pag-seg' ><div class='input-group input-group-sm'><span class='input-group-text change-span-device'>RD$</span><input type='text' class='total-pag-seg form-control' value='0.00' readonly /></div></td><td class='descuento'><div class='input-group input-group-sm'><span class='input-group-text change-span-device'>RD$</span><input name='descuento" + numRows + "' type='text' class='descuento form-control float text-danger'  tabindex='" + (ti++) + "'onkeypress='return onlyfloat(event);' /></div></td><td class='total-pag-pa'><div class='input-group input-group-sm'><span class='input-group-text change-span-device'>RD$</span><input  name='totalpagpa" + numRows + "' type='text' class='total-pag-pa form-control ' value='0.00' tabindex='" + (ti++) + "' readonly /></div></td><td><input type='checkbox' name='chkbox' class='remove'></td>");
	
   var areaResults = $(".get-tarif-data > option").clone();
   $('.get-tarif-data-clone').append(areaResults);
	...
	$('#save_factura').on('click', function(event) {
		...
		var grupo_area = $('#center_tariff_area').val();
		var service = [];
		var cantidad = [];
		var precio = [];
		var subtotal = [];
       ...
       $('input.total-pag-seg').each(function() {
		...
		$.ajax({
			type: "POST",
			dataType: 'json',
			url: "<?= base_url('patient_factura_controller/saveInvoiceCentroPuplico') ?>",
			data: {
			...
			error: function(jqXHR, textStatus, errorThrown) {
				...
				$('#servFieldfsdfsdf').html('<p>status code: ' + jqXHR.status + '</p><p>errorThrown: ' + errorThrown + '</p><p>jqXHR.responseText:</p><div>' + jqXHR.responseText + '</div>');
				console.log('jqXHR:');
				console.log(jqXHR);
