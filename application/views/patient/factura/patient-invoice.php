<section class="py-3">
		...
		<div class="offcanvas offcanvas-end " style="width:350px" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="offcanvasScrollingAbono" aria-labelledby="offcanvasScrollingLabel">
          <div class="offcanvas-header border-bottom">
            <h5 class="offcanvas-title" id="offcanvasScrollingLabel">Agregar Abono a la Factura </h5>
          ...
          <div class="offcanvas-body">
						...
						<div class="form-floating mb-3">
						<input type="date" class="form-control" id="fecha-bono" placeholder="name@example.com">
						<label for="fecha-bono">Fecha</label>
						</div>
					...
					<label for="monto-bono">Abono</label>
					</div>

					</div>
					
				<button class='btn btn-sm btn-danger float-end m-1' id="save-abono" type="button">Guardar</button>
					
                         <span id="bono-result"></span>  
						<div id="list-abono"></div>
          </div>
        </div>
	 ...
	 if($tipo_monena=="RD$"){ ?>
				...
				if($values_container_decrpyted['seguroId'] !=11){
					$print_page_invoice = "patient_invoice";
					$show_abono = 'style="display:none"';
				}else{
				$print_page_invoice = "patient_invoice_private";	
				$show_abono = '';
				}
				...
				if($tipo_tarifario=='Doctor Tarifarios'){
				?>
				
				<button type="button" class="btn btn-sm btn-danger button_delete_tarifarios float-end m-2" title="Eliminar la factura #<?=$idfacc?>"  type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrolling" aria-controls="offcanvasScrolling" ><i class="bi bi-x-circle-fill"></i> Eliminar</button>
				<?php } elseif($tipo_tarifario=='Centro Tarifarios' && $this->session->userdata('user_perfil') !="Asistente Medico") { ?>
				<button type="button" class="btn btn-sm btn-danger button_delete_tarifarios float-end m-2" title="Eliminar la factura #<?=$idfacc?>"  type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrolling" aria-controls="offcanvasScrolling" ><i class="bi bi-x-circle-fill"></i> Eliminar</button>
				<?php }  ?>
				  <form  target="_blank" METHOD="GET" ACTION="<?php echo site_url("print_page/$print_page_invoice"); ?>">
       <input name="idfacc" type="hidden" value="<?=$idfacc?>" /> 
		  <input name="print" type="hidden" value="<?=$print?>" />
		 <input name="seguro" type="hidden" value="<?=$values_container_decrpyted['seguroId'] ?>" />
			<button  type="submit" class="btn btn-primary btn-sm float-end m-2" ><i class="bi bi-printer"></i> Imprimir </button>
		   </form>
		   
		    	<form target="_blank" METHOD="GET" ACTION="<?php echo site_url("print_ticket/$print_page_invoice"); ?>">
							<input name="idfacc" type="hidden" value="<?= $idfacc ?>" />
							<input name="print" type="hidden" value="<?= $print ?>" />
							<button type="submit" class="btn btn-primary btn-sm float-end m-2"><i class="bi bi-printer"></i> Imprimir en Ticket </button>
						</form>
						<button type="button" <?=$show_abono?> class="btn btn-sm btn-primary  float-end m-2 fetchPatientAbono"   type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrollingAbono" aria-controls="offcanvasScrollingAbono" ><i class="bi bi-currency-dollar"></i> Abono</button>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-sm table-striped mb-0 text-start" id="table-tarifario" style="font-size:12px">
						
							
                            <thead>
								<?php
								$area_public_centro_asegurado=$this->db->select('area_public_centro_asegurado')->where('idfacc',$idfacc)->get('factura2')->row('area_public_centro_asegurado');
								if($tipo_tarifario=='Centro Tarifarios' && $values_container_decrpyted['seguroId'] !=11){?>
								<tr>
								<td>
								<div class="input-group input-group-sm">
										<span class="input-group-text change-span-device" id="basic-addon2"><strong>Area</strong></span>
										<input disabled type="text" class="form-control" value="<?=$area_public_centro_asegurado?>">
									</div>
								...
								<?php }  ?>
                                <tr class="bg-th">
								<?php if($tipo_tarifario=='Centro Tarifarios' && $values_container_decrpyted['seguroId'] ==11){ ?>
								<th >
									Area
								</th>
								<?php } ?>
...
<input id="id_facc" value="<?=$idfacc?>" type="hidden" />
 <input id="patient_seguro_from_factura" value="<?=$factura_row1['seguro_medico']?>" type="hidden" />
  ...
  <input id="patient_patId" value="<?=$patId?>" type="hidden" />
...
$('.fetchPatientAbono').on('click', function () {
 listarFacturaAbono();
});
...
{
$("#list-abono").fadeIn().html('<span  class="glyphicon glyphicon-refresh glyphicon-refresh-animate load"></span>');
$.ajax({
type: "POST",
url: "<?=base_url('patient_abono/listarFacturaAbono')?>",
data: {id_facc:<?=$idfacc?>},
success:function(data){ 
$("#list-abono").html(data);
}  
})
  ...
  $('#save-abono').on('click', function(event) {
...
$.ajax({
type: "POST",
url: "<?=base_url('patient_abono/saveFacturaBono')?>",
data: {fecha:fecha,bono:bono,id_facc:<?=$idfacc?>,bonoResta:bonoResta},
success:function(data){
...
$('#bono-result').html('<p>status code: '+jqXHR.status+'</p><p>errorThrown: ' + errorThrown + '</p><p>jqXHR.responseText:</p><div>'+jqXHR.responseText + '</div>');
console.log('jqXHR:');
console.log(jqXHR);
...
data: {id_facc:$("#id_facc").val(), patient_seguro_from_factura:$("#patient_seguro_from_factura").val(), centro_type:$("#centro_type").val(),privado:1, id_pat:$("#patient_patId").val()},
success:function(data){ 
