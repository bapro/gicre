...
<?php
 ...
 $pm = $this->db->select('paciente,medico,centro_medico,id_rdv')->where('idfacc', $row1->idfacc)->get('factura2')->row_array();
        ...
        $lastPatConDiag = $this->db->select('id, procedimiento, otros_diagnos')->where('historial_id', $row1->paciente)->where('id_user', $row1->medico)->order_by('id', 'DESC')->get('h_c_conclusion_diagno')->row_array();
			if($lastPatConDiag){
...
<tr>
<td style="text-align:right" valign="top"><strong>AREA(S)</strong> :</td>

<td style="text-align:left">

<?php
$query= $this->db->query("SELECT idfac, grupo_area, service FROM factura WHERE id2=$row1->idfacc GROUP BY grupo_area");
if($print=='medico'){
