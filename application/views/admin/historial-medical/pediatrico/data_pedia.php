...
$data=$this->db->select('*')->where('hist_id',$id_historial)->get('h_c_ant_pedia')->row_array();
$insed_time = date("d-m-Y H:i:s", strtotime($data['inserted_time']));
$upda_time = date("d-m-Y H:i:s", strtotime($data['updated_time']));
?>
<br/>
<h4 class=" h4 his_med_title"  >Antecedentes Perinatal </h4>
...
<div class="col-md-8">
<select  class="form-control select2" multiple="multiple"  id="med1" >
<?php 

foreach($querym->result() as $med)
{ 

$medica=$this->db->select('medica')
->where('medica',$med->id)->where('hist_id',$id_historial)
