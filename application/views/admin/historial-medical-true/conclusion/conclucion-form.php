...
<textarea rows="6" class="form-control required-patient-inputs" id="plan" ></textarea>
</div>
...
<div class="col-md-10">
 <button type="button" id="btnSpeechConDiagProced" title='soporte solo para el navegador Chrome'  class="btn btn-primary btn-sm" ><i class="fa fa-microphone" aria-hidden="true"></i></button>
  ...
  <textarea rows="6" class="form-control" id="proced" ></textarea>
  <?php if($id_seg){?>
<h4><?=$seg_n?></h4>
...
foreach($proct->result() as $row)
{ 
echo '<option value="'.$row->id_tarif.'">'.$row->procedimiento.' (RD$'.$row->monto.')</option>';
}
?>
  </select>
<?php }else{
...
<textarea rows="7" class="form-control required-patient-inputs" id="evolucion"  ></textarea>
</div>
