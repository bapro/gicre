...
$user_c12=$this->db->select('name')->where('id_user',$row->inserted_by)->get('users')->row('name');
$user_c13=$this->db->select('name')->where('id_user',$row->updated_by)->get('users')->row('name');
...
?>
<br/>
<h4 class=" h4 his_med_title"  >Antecedentes Perinatal
 <a style="cursor:pointer" id="edit_ped" class="btn-sm" >
<i class="fa fa-pencil" aria-hidden="true"></i> Editar</a>
<a style="cursor:pointer;color:green;display:none" id="save_ant_ped"  ><i class="fa fa-floppy-o" aria-hidden="true"></i> Guardar</a>
<a data-toggle="modal"  data-target="#print_pedia"  href="<?php echo base_url("printings/print_if_foto/$row->id/0/0/pedia")?>"  style="cursor:pointer" title="Imprimir Antecedentes General" class="btn-sm" ><i style="font-size:24px" class="fa">&#xf02f;</i></a>

</h4>
<h5 class="modal-title text-center"  >Creado por :<?=$user_c12 ?> | fecha :<?=$insed_time?> | <span style="color:red"> Cambiado por :<?=$user_c13?> | fecha :<?=$upda_time?></span> </h5>
<hr class="title-highline-top"/>
<span class="disable-all">
...
<tr>
<td colspan="6">
<textarea class="form-control" cols="170" id="evol_text1"><?=$row->evol_text?></textarea>
...
<select class="form-control" id="edad_gest1">
...
<div id="load-patient-medicine-pedia"></div>
</div>
<div class="col-md-6" >

	<input class="form-control selectpedia"   placeholder='Buscar neuvo medicamento' >
	<br/>
...
<?=$row->maltrato?></textarea></td>
 ...
 <?php  
   $sql ="SELECT * FROM h_c_vacunacion join patients_appointments on h_c_vacunacion.hist_id=patients_appointments.id_p_a
 where hist_id=$id_historial";
$query = $this->db->query($sql);
$i = 1;
...
<h4 style="color:green">VACUNACION </h4>Si <span style="color:red"><i>Dias de atraso</i></span> esta diferente de 0 hay una fecha aplicada.
</div> 
<div class="col-md-12" style="overflow-x:auto;"  >  
 <table class="table table-striped table-bordered" id="disab-vacu" style="background:#E6E6FA;">
...
<a  href="#" title="Ve arriba" style="cursor:pointer"><i class="fa fa-arrow-up" aria-hidden="true" style="font-size:24px"></i></a>
<br/><br/><br/>
<div class="modal fade" id="print_pedia"  role="dialog" >
...
$this->load->view('admin/historial-medical1/pediatrico/index');
