...
<?php $nombresp=$this->db->select('nombre')->where('id_p_a',$id_historial)->get('patients_appointments')->row('nombre');?>
<button type="button" title="Cierra" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times-circle"  style="font-size:48px;color:red"></i></button>
<h3 class="h3 his_med_title">REFRACCION - <?=$nombresp?></h3>
<hr class="title-highline-top"/>
</div>
<div class="modal-body" id="background_">

 <div id="paginationRefraction"></div>
...
<input name='id_lab_lente' type='hidden' value="<?=$id_lab_lente?>"/>
...
//---------------------------------------------------------------------------------------------------------------
function paginationRefraction(){
$('#paginationRefraction').html('cargando...').css({'font-style' : 'italic'});
$.ajax({
url:"<?php echo base_url(); ?>admin_medico/paginationRefraction",
data: {id_user:<?=$user_id?>,id_patient:<?=$id_historial?>},
method:"POST",
success:function(data){
$('#paginationRefraction').html(data);
},
error:function(jqXHR, textStatus, errorThrown) {
alert('An erroroccurred... Look at the console (F12 or Ctrl+Shift+I, Console tab) for more information!');
...
$('#save-of-ref-text').prop("disabled", false);
//$('#new-id-ref').val(data);
