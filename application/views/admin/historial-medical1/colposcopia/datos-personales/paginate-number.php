...
<div id="paginationh" class="colposcopy-pagination">
...
<div class="load-paginate-on-click" id="load-colposcopy"></div>

<script>

$(".colposcopy-pagination li").click(function(){
$("#load-colposcopy").show().html('<span style="font-size:18px" class="glyphicon glyphicon-refresh glyphicon-refresh-animate load"></span>');
 $('#colcoscopyContentDatosPersonales').css("opacity",".5");
$("#paginationh li").css({'border' : 'solid #0063DC 2px'}).css({'color' : '#0063DC'});

$(this).css({'color' : '#FF0084'}).css({'border' : 'none'});
//Loading Data
var pageNum = this.id;
$("#colcoscopyContentDatosPersonales").show();
$("#colcoscopyContentDatosPersonales").load("<?php echo base_url(); ?>/Colcoscopy_content/paginationData?page="+pageNum+"&idUser="+<?=$user_id?>+"&id_patient="+<?=$idpatient?>+"&id_centro="+<?=$id_centro?>);
