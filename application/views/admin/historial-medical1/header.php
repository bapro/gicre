...
//------------------------------
$today=date('Y-m-d');
$sql_con ="SELECT historial_id FROM h_c_enfermedad where filter_date='$today' AND historial_id=$id_historial AND user_id=$user_id AND centro_medico=$centro_medico";
$atendido_con = $this->db->query($sql_con);

if($atendido_con->num_rows() ==1){
$disable_save="disabled";
$nocreatecita='<span style="color:green"><i style="color:green" class="fa fa-check" title="Historial clínica esta creada por hoy."></i> </span>';
$info="";
}  else if($hist=="" || $hist==4 || $historial_id=="" || $user_id=="" || $areaid==""){
$disable_save="disabled";
$nocreatecita="";
$info="<span style='color:#B22222;font-size:10px'>no puede guardar</span>";
}
else{
$disable_save="disabled";
$nocreatecita="en modo vista";
$info="";
}
...
?>
<a target="_blank" href="<?php echo site_url("$controller/historial_medical/$tut->id_tutor/$user_id/$areaid/$centro_medico/2"); ?>"><strong><?=$tut->relacion?></strong> : <?=$tut->name_tutor?></a>
</li>
<?php
}
?>
...
<li><a><?=$info?> <button <?=$disable_save?>  class="btn btn-md btn-success save_ant_gen" id="save_ant_gen"  type="submit"><i class="fa fa-floppy-o" aria-hidden="true"></i> Guardar </button></a></li>
