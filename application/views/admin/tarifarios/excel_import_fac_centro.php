...
$seguro=$this->db->select('title')->where('id_sm',$id_seg)->get('seguro_medico')->row('title');

$if_empty=$this->db->select('centro_id')->where('centro_id',$id_c)->where('seguro_id',$id_seg)->get('centros_tarifarios')->row('centro_id');
if($if_empty=="")
{
	$hide="";
...
<div class="col-sm-4">
<input name="id_sm" value="<?=$id_seg?>" type="hidden"> 
<?=$seguro?>
</div>
</div>
