...
$condition2 = "fecha BETWEEN " . "'" . $desde . "'" . " AND " . "'" . $hasta. "'";
 $sqlncf ="SELECT *
	 FROM 
	 invoice_ars_claims 
 ...
 $ncf_id=$nfc['id_ncf']; 
  $update_date= date("d-m-Y H:i",strtotime($n->updated_date));
   $crpa=$this->db->select('name')->where('id_user',$n->updated_by)->get('users')->row('name');
	 ?>
	 <div class="alert alert-info">Hay <strong><u>NCF : <?=$nfc['value']?></u></strong> con <strong><?=$query->num_rows()?> factura(s)</strong> creada(s) segun las entradas de la busqueda (Creado por : <?=$crpa?> | <?=$update_date?>)<a  target="_blank" href="<?php echo base_url("admin_medico/invoice_ars_p_v_/0/0/$ncf_id/$is_admin")?>" style="cursor:pointer" title="Imprimir" class="btn btn-sm" >Ver</a></div>  
	 <?php 
		 } 
     }
		 $sql2 ="SELECT * FROM factura WHERE  $condition AND pat_id= '$patient'  AND idfac NOT IN (SELECT id_fac2 from invoice_ars_claims)";
