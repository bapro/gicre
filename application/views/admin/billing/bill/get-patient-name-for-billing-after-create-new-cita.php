...
$cod=$this->db->select('codigo')->where('id_seguro',$row->seguro_medico)->where('id_doctor',$doc->doctor)
->get('codigo_contrato')->row('codigo');
$centro=$this->db->select('name')->where('id_m_c',$doc->centro)->get('medical_centers')->row('name');
?>
...
<th class="heading">
<?php if(empty($serv)) {
	echo "<span style='color:#B18904'>No hay servicios | <a  href='".base_url()."/admin/import_rates_fac/$doc->doctor/$row->seguro_medico/medico'>Crear</a></span>";
