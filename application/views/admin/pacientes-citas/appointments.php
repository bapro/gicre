...
<select id="centro_medico" name="centro_medico"  class="form-control" style="background:#E0E5E6;">
 <option value="Todos los centros médicos">Elegir un centro médicos</option>
 <?php 
...
// display centro medico
  $(document).ready(function(){
    ...
    $( '#centro_medico' ).on('change', function(){
		  $("#flash1").fadeIn(400).html('<img style="width:60px" src="<?= base_url();?>assets/img/loading.gif" />');
           var exam="1";
      ...
      $.ajax({
         ...
         // data:'centro_medico='+$("#centro_medico").val(),
		  data: {centro_medico:$("#centro_medico").val(),exam:exam},
    ...
    e.preventDefault();
	$("#flash1").fadeIn(400).html('<span class="load"><img style="width:60px"  src="<?= base_url();?>assets/img/loading.gif" /></span>');
    var exam="2";
    $.ajax({
        url: '<?php echo site_url('admin/get_centro_medico_datepicker');?>',
        type: 'post',
        data: {date_from:$("#date_from").val(),date_to:$("#date_to").val(),exam:exam},
        success: function (data) {
			$("#centro_medico").val("");
               $("#flash1").hide();
