...
<?php if($row->provincia==""){
	echo "<div class='alert alert-warning'><i class='fa fa-warning' style='font-size:24px'></i> Este paciente viene de una solicitud faltan algunas informaciones</div>";
	$red_color="red";
	}else{
	$red_color="";	
	}?>
...
echo '<img  style="width:100%;height:auto;" src="'.base_url().'/assets/img/user.png"  />';	
}
$readonlyag="readonly";
} 
else if($row->photo==""){
...
<li>
<a class="remb" href="#Datos_citas" data-toggle="tab">IV- DATOS CITAS <span style="color:black">(<?=$number_cita?>)</span> <br/><?=$nueva_cita?></a>
</li>

<li <?=$display_optometra?>><a class="remb" href="#FACTURAR" data-toggle="tab">V- FACTURA AMBULATORIA <span style="color:red;font-size:13px"><br/>Si desea facturar sin realizar citas use esta opción</span></a></li>
...
<?php if($is_hist_med !="") {
 if($perfil=="Admin") {?>
<li <?=$display_optometra?>><a href="<?php echo site_url("historial_medical/$row->id_p_a/$id_user/0/$id_cm/1/$id_user"); ?>">HISTORIAL MEDICA</a></li>
<?php } else if($perfil=="Medico"){
?>
<li <?=$display_optometra?>><a href="<?php echo site_url("$controller/historial_medical_past/$row->id_p_a/$id_user/$area/$id_cm/1/$id_user"); ?>">HISTORIAL MEDICA</a></li>
<?php	
}else if($perfil=="Asistente Medico"){
$areaid= $this->db->select('area')->where('id_user',$doc)->get('users')->row('area');
?>
<li <?=$display_optometra?>><a href="<?php echo site_url("$controller/historial_medical_past/$row->id_p_a/$doc/$areaid/$id_cm/4/$id_user"); ?>">HISTORIAL MEDICA </a></li>
<?php
} }?>
<li <?=$display_optometra?>><a href="<?php echo base_url("$controller/orden_medica/$id_user/$row->id_p_a")?>">ORDEN MEDICA </a></li>
...
<button type="submit" id="sendc" style="float:right;display:none" class="btn btn-success btn-sm save-patient enable-send" ><i class="fa fa-floppy-o" aria-hidden="true" title="Guardar  <?=$row->nombre?>" ></i> Guardar</button>
</span>
</h4>
...
<input type="text" class="form-control required" id="date_nacer" name="date_nacer" value="<?=$row->date_nacer?>" <?=$readonlyag?>/>
...
<?php } ?>
<div class="form-group">
<label class="control-label col-sm-3" style="color:<?=$red_color?>">Provincia</label>
...
<div class="form-group">
<label class="control-label col-sm-3" style="color:<?=$red_color?>">Municipio</label>
<div class="col-sm-7">
...
<div class="col-sm-6">
<?php 
$firma_doc="$row->id_p_a-0.png";

$signature = "assets/update/$firma_doc";

if (file_exists($signature)) {?>
<img  class="img-fluid" style="border:1px solid #38a7bb;width:100%" src="<?= base_url();?>/assets/update/<?=$firma_doc?>"  />
<?php
} else {
	?>
  <a style="color:red;font-size:14px" href="<?php echo site_url("printings/signature/$row->id_p_a/3/$id_cm/$doc/$controller");?>"><i>Crear Firma</i></a>
<?php
}
?>
...
if($seguro_medico['title']=="PRIVADO"){
 $plan_id=0; $plan='';	
} else {
	if($row->plan){
...
<input type="radio" name="seguro-radio" value="<?=$row->seguro_medico?>" checked> <?=$seguro_medico['title']?>
</label>
<label class="checkbox-inline">
...
/*$('#facturar-doc').select2({ 
placeholder: "CENTRO MEDICO",
tags: true
}).on('change.facturar-doc', function(e){
var doc = $(this).val();
var seg=<?=$row->seguro_medico?>;
var id=<?=$row->id_p_a?>;
var centro= $("#factura-centro").val();
if (doc) { 
window.location ="<?php echo base_url(); ?><?=$controller?>/fact?centro="+centro+"&seg="+seg+"&id="+id+"&doc="+doc; 
}
return false;
});*/
...
type: 'post',
data:{idpatient:idpatient,seguro_medico:seguro_medico,plan:<?=$plan_id?>},
success: function (data) {
