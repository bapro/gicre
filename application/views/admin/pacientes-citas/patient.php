...
if($centroType=="Salud ocupacional"){
...
<li><a  data-target="#verEmpData" href="<?php echo site_url("zona_franca/getClockResultPatient/$row->id_p_a/$id_user/$id_cm")?>" data-toggle="modal">FICHA DE EMPLEADO</a></li>
<li><a class="remb" data-target="#verMedSo" href="<?php echo site_url("salud_ocupacional_med/getMedicamentoSalOcup/$row->id_p_a/$id_user/$id_cm")?>" data-toggle="modal">DRUG</a></li>
<?php } 
else{ ?>
<li <?=$zona_franca?>>
<a>
...
<li <?=$display_optometra?> <?=$zona_franca?>><a class="remb showFactAmb" href="#FACTURAR" data-toggle="tab">FACTURA AMBULATORIA <span style="color:red;font-size:13px"><br/>Si desea facturar sin realizar citas use esta opción</span></a></li>

<!--<li <?=$display_optometra?>><a  class="remb" data-toggle="tab" href="#emergencia">VI- EMERGENCIA (<?=$number_patient_admitas?>)</a></li>-->
<li <?=$display_optometra?> <?=$zona_franca?> ><a  class="remb" data-toggle="tab" href="#hospitalizacion">HOSTIPITALIZACION </a></li>
<?php if($is_hist_med !="") {
 if($perfil=="Admin") {
...
<input type="hidden" name="table_name"  id="table_name" value="patients_folders" />
 ...
 <input name="insertedBy"  type="hidden" value="<?=$id_user?>" />
  <input id="user_perfil"  type="hidden" value="<?=$perfil?>" />
...
<script src="<?=base_url();?>assets/js/upload-foto.js?rnd=3033"></script> 
...
$('#savePatEmp').on('submit', function (e) {
	e.preventDefault();
$.ajax({
url: "<?=base_url('zona_franca/savePatientEmp')?>", // Url to which the request is send
