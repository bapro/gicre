...
<option value=''>Seleccionar o crear un groupo </option>

	<?php 
	 $sqllbb ="SELECT * FROM h_c_groupo_estudios  GROUP BY groupo ORDER BY id DESC";
...
<th style="width:5px">Estudios</th>
...
$.ajax({
type: "POST",
url: "<?=base_url('admin_medico/nuevoEstudio')?>",
 ...
 $('#nuevo-groupo').on('change', function(event) {
  ...
  $('.check-lab').on('change', function(event) {
   ...
   if ($(this).is(':checked')) {
	...
	$.ajax({
		type:'POST',
		url:'<?=base_url('admin_medico/nuevoEstGroupo')?>',
	...
	$.ajax({
		type:'POST',
		url:'<?=base_url('admin_medico/deleteEstGroupo')?>',
		data: {lab:lab,nuevogroupo:nuevogroupo},
		success:function(data) {
...
$.ajax({
type:'POST',
url: "<?=base_url('admin_medico/edit_est')?>",
dataType: "json",
...
$.ajax({
type:'POST',
url:'<?=base_url('admin_medico/deleteEst')?>',
data: {id : del_id},
