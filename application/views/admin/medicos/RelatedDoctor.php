...
{
echo"<div class='alert alert-warning'> No hay Doctores relacionados con <span style='font-weight:bold'>$area</span> . </div>";
?>
...
<table id="message-table-doc"  class="table table-striped table-bordered message-table-doc" width="100%" cellspacing="0">
    <thead>
    ...
    <?php foreach($relatedDoctor as $all_m_c)
	 ...
	 ?>
        <tr>
		    <td style='display:none' class="id_doc"><?=$all_m_c->id_user;?></td>
            <td style='text-transform:uppercase'><?=$all_m_c->name;?></td>
            <td><?=$all_m_c->cell_phone;?></td>
           <td><?=$all_m_c->correo;?></td>
	 ...
	 
	 {
        ...
        <tr>
          <th><?=wordwrap($mesa,10,"\n", 1)?></th>
           ...
           <td><?= $insert_time?> <a title="Eliminar" id="<?=$mes->idsm; ?>"  class="delete" style="color:rgb(223,0,0)"><i class="fa fa-trash-o" aria-hidden="true" ></i></a></td>
	  </tr>
	  	<?php	
	 }
	 ?>
...
$(document).ready(function(){
 ...
 $("#bt").submit(function(e){
	...
	var iduser=<?=$iduser?>;
	var doc = [];
	$("#message-table-doc").find('td.id_doc').each(function(){
    doc.push($(this).text());
    });
	  $.ajax({
		...
		data: {idarea:idarea,message:message,doc:doc,iduser:iduser},
