...
$user_c1=$this->db->select('name')->where('id_user',$row->operator)->get('users')->row('name');
$user_c2=$this->db->select('name')->where('id_user',$row->update_by)->get('users')->row('name');
?>
...
<a style="cursor:pointer" id="edit_datam" class="btn-sm" >
...
<a data-toggle="modal"  data-target="#print_gnl_foto"  href="<?php echo base_url("printings/print_if_foto/$historial_id/$areaid/$user_id/gnl")?>"  style="cursor:pointer" title="Imprimir Antecedentes General" class="btn-sm" ><i style="font-size:24px" class="fa">&#xf02f;</i></a>
</th>
</tr>
...
$user_c6=$this->db->select('name')->where('id_user',$row->update_by)->get('users')->row('name');
?>
 <tr>
<td colspan="1">
...
<th>
<a style="cursor:pointer;color:green"  class=" btn-sm" id="save_edit_otrosant"  ><i class="fa fa-floppy-o" aria-hidden="true"></i> Guardar</a>

<a style="cursor:pointer;" id="edit_dataotrosant" class="btn-sm" ><i class="fa fa-pencil" aria-hidden="true"></i> Editar</a>
<i title="Ingresado por <?=$user_c5?> a <?=$row->inserted_time?> &#013 Modificado por <?=$user_c6?> a <?=$row->update_time?>" class="fa fa-info-circle" aria-hidden="true"></i>
</th>
</tr>
...
<?php foreach($medicamentos as $rowch)
$id_p=$this->db->select('medicine')
->where('id_patient',$id_historial)
//->where('medicine',$rowch->name)
...
$chy="checked";	
 ...
 ?>
<span class="ninguno1">No&nbsp;<input type="radio" id="radiomedi" name="radiomedi" <?=$ch?> disabled>&nbsp; Si&nbsp;<input type="radio" id="radiomedi" name="radiomedi" class="chM" <?=$chy?> disabled></span>
<select class="form-control select2"  multiple="multiple" id="selectmedic" disabled >

<?php 
foreach($medicamentos as $rowmeds)
{ 
$id_pv=$this->db->select('medicine')
->where('id_patient',$id_historial)
->where('medicine',$rowmeds->name)
->get('h_c_patient_medicine')->row('medicine');

if($rowmeds->name==$id_pv){
	$selected="selected";
} else {
   $selected="";
}
echo "<option value='$rowmeds->name' $selected>$rowmeds->name</option>";
}
?>
</select>
	</td>
...
echo "<label>&nbsp;Positivo&nbsp;<input type='radio' id='rhch' class='ck' name='rhch' value='Positivo' $checked></label>";
...
echo "<label>&nbsp; &nbsp; Negativo&nbsp;<input type='radio' id='rhch' name='rhch' value='Negativo'  $checked></label>";
?>
...
<div id="load-patient-medicine"></div>
<input class="form-control selectmedic"   placeholder='Buscar neuvo medicamento' disabled>
<input class="form-control medica-time" type="hidden"  value="2" >
	<br/>
