...
?>
 <form class="form-horizontal" method="post" id="change-bgd-pag-re" >
  <h5 style="color:#FF0084">REGISTRO #<?=$page?> | creado por <?=$doc?>, <?=$inserted_time?></h5>
  	<a title="Borrar" class="btn btn-md btn-danger deleteRepGen"   id="<?=$rowp->id; ?>"  style="color:red;cursor:pointer"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
	...
	<button type="button" id="save-cirugia-generales" class="btn btn-md btn-success"><i class="fa fa-check after-action" style="display:none;color:blue;font-size:35px;position:absolute"></i> CAMBIAR</button>
		 <a  class="btn btn-md btn-primary"  target="_blank" href="<?php echo base_url("printings/print_if_foto_c/$rowp->id/$id_patient/$user_id/$rowp->id_centro/r")?>"  ><i class="fa fa-print"></i> Imprimir Vert.</a>
      <a  class="btn btn-md btn-primary"  target="_blank" href="<?php echo base_url("printings/print_if_foto_c1/$rowp->id/$id_patient/$user_id/$rowp->id_centro/r")?>"  ><i class="fa fa-print"></i> Imprimir Horiz.</a>
	  </div>
    </div>
...
$(".deleteRepGen").on('click', function(event) {
...
$.ajax({
type:'POST',
url:'<?=base_url('historialHeader/deleteReportGeneral')?>',
data: {id : del_id},
success:function(response) {
$("#contentrp").hide(); $("#hide-form-cirugia-reporte").show();
