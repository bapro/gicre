...
<div class="col-md-10">
<textarea  class="form-control" rows="8"  id="otros_diagnos1" disabled><?=$otros_diag?></textarea>
</div>
</div>
...
<h4><?=$seg_nd?></h4>
  ...
  <?php 
  $sqlptd = "select id_tarif, procedimiento,monto FROM tarifarios WHERE id_seguro=$id_segd AND id_doctor=$row->inserted_by GROUP BY procedimiento";
...
$.ajax({
url:"<?php echo base_url(); ?>factura/save_hist_tarif_pro",
data: {id_tarif:$(this).val(),id_cd:<?=$row->id_cdia?>,id_historial:<?=$row->historial_id?>, user_id:<?=$user_id?>,centro:<?=$centro_medico?>},
method:"POST",
...
$.ajax({
url:"<?php echo base_url(); ?>factura/loadProcedimientoFacData",
data: {id_cd:<?=$row->id_cdia?>,patient:<?=$row->historial_id?>, user_id:<?=$user_id?>,centro:<?=$centro_medico?>},
method:"POST",
success:function(data){
$("#loadProcedimientoFacData").html(data);
},
