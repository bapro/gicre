...
$sqltr ="SELECT id,inserted_time,inserted_by FROM  hosp_balance_hidrico_t72 WHERE $contition id_patient=$id_patient ORDER BY id DESC ";
...
<script>
$("#paginationh72 li").on('click', function(e){
  e.preventDefault();
$('#content-turno-paginate-72').css("opacity",".5");
$("#SaveTurno72").css("opacity",".5");
$(".paginationh li").css({'border' : 'solid #0063DC 2px'}).css({'color' : '#0063DC'});
$(this).css({'color' : '#FF0084'}).css({'border' : 'none'});
//Loading Data
var pageNum = this.id;
$("#content-turno-paginate-72").show();
$('.successfull_saving').html(' procesando...').addClass('fa fa-spinner');
$.ajax({
type: "GET",
url: "<?=base_url('hosp_balance_hidrico/pagination_data_turno72')?>",
data: {page:pageNum,user_id:<?=$id_user?>,id_patient:<?=$id_patient?>,perfil:"<?=$perfil?>"},
 success:function(data){ 
$('#content-turno-paginate-72').html(data);
$('#content-turno-paginate-72').css("opacity","");
