...
<div class="input-group" style="font-size:5px;font-style:italic">
<span class="input-group-addon text-response"></span>
</div>
...
data: {id_patient:id_patient,id_user:id_user,enviado_a:<?=$enviado_a?>},
method:"POST",
...
data: {selected:selected,responder:responder.trim(),id_user:<?=$user_id?>},
method:"POST",
success:function(data){
	$("#show-selected").text("");
$('#responder-text').val('');
...
{
$('#add-interconsulta').prop("disabled",false);
var id_user  = <?=$user_id?>;
var id_patient = <?=$patient_id?>;
$.ajax({
