...
<button type="button" title="Cierra" class="close close-edit-med" aria-hidden="true"><i class="fa fa-times-circle"  style="font-size:48px;color:red"></i></button>
...
Creado por :<?=$ussss['name']?> | fecha :<?=$inserted_time?>
  ...
  <div class='text-center' id='done-edit-ord'><strong></strong></div>
</div>
...
<div class="col-md-9">
<textarea  class="form-control reset-est" id="descripcion_mg2" rows='6' ><?=$row_edit->descripcion_mg?></textarea>
</div>
</div>
...
$('.close-edit-med').on('click', function () {
...
$.ajax({
type: "POST",
url: "<?=base_url('hospitalizacion/updateOrdMedSave')?>",
...
success:function(data){
 $("#done-edit-ord").html(data);
setTimeout(function() {$('#edit_medida_gnl').modal('hide');}, 1000);
}
});
return false;
});
