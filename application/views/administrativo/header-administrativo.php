...
<noscript><meta http-equiv="refresh" content="0; url=<?php echo site_url('administrativo_medico/noJs');?>" /></noscript>
...
if(second <= 0){
window.location.href="<?php echo base_url(); ?>/login/administrativo_logout";
} else{
...
$name=$this->session->userdata['administrativo_name'];
 if(empty($this->session->userdata['administrativo_name'])){
redirect('https://www.admedicall.com');	
 }
$id_usr=$this->session->userdata['administrativo_id'];
?>
...
<li><a href="<?php echo site_url('administrativo/searchChart');?>" style="color:red"><i class="fa fa-bar-chart" style=""></i> Gráfico</a></li>
<li><a  href="<?php echo base_url('administrativo/chatBox')?>" ><i class='fa fa-comments'></i> Chatear</a></li>
...
<ul class="nav navbar-nav" style="margin-left:10%"  >
 ...
 <?php
 $this->load->view('administrativo/view_acciones');
...
<li class="dropdown" >
  ...
  <a title="<?= $name; ?> : <?= $centroname; ?>" class="dropdown-toggle" style="cursor:pointer" data-toggle="dropdown" data-hover="dropdown">
  <img src="<?= base_url();?>assets/img/user.png" style="width:25px;border-radius:20px" alt=""/> <?= $name; ?> : <?= substr($centroname, 0, 10); ?>...
 ...
 <li><a href="<?php echo site_url('administrativo/users');?>"> <img src="<?= base_url();?>assets/img/user.png" style="width:25px;border-radius:20px" alt=""/> <?=$query_user->num_rows();?> Usuarios | <span id='connected_users'></span> conectado(s)</a></li>
  <li><a href="<?php echo site_url('administrativo/payment_received');?>" ><i class="fa fa-money" style="color:red"></i> Pagos</a></li>
 <li><a href="<?php echo site_url('login/administrativo_logout');?>" ><i class="fa fa-sign-out"></i>  Cerrar sesión</a></li>
...
<script>
 setInterval(function(){
  $('#connected_users').load("<?php echo base_url('administrativo/current_user_login')?>").fadeIn("slow");
 }, 1000);
setInterval(function(){
  $('#new_message').load("<?php echo base_url('administrativo/newMessageReceived')?>").fadeIn("slow");
 }, 1000);
