...
if($display_report !=NULL){
 ...
 $paciente=$this->db->select('id_p_a,nombre,tel_resi,tel_cel,email,afiliado,cedula,photo,ced1,ced2,ced3,plan,seguro_medico')->where('id_p_a',$row1->paciente)
...
$logoseg='<img  style="width:100px;" src="'.base_url().'/assets/img/seguros_medicos/'.$seguron['logo'].'"  />';	
}
 $seguroCodDoc=$this->db->select('codigo')->where('id_seguro',$seguro)->where('id_doctor',$row1->medico)->get('codigo_contrato')->row('codigo');
 
 $seguroCodCentro=$this->db->select('codigo')->where('id_centro',$row1->centro_medico)->where('id_seguro',$seguro)->get('codigo_contrato')->row('codigo');
...
<h4 style="text-align:center;text-transform:uppercase"><?=$headInfo?></h4>
<?=$headInfo_?>
<span style="color:blue;font-size:13px">RECLAMACION DE PAGO POR SERVICIO PRESTADO</span><br/>
<span style="font-size:10px"><?=$logoseg?></span><br/>
...
<td style="text-align:right"><strong>AUTORIZADO POR</strong> :</td><td style="text-align:left"><?=$row1->autopor?></td>
 ...
 <?php } ?>
<tr>
<td style="text-align:right"><strong>FECHA</strong> :</td><td style="text-align:left"><?=date("d-m-Y", strtotime($row1->fecha))?></td>
</tr>
...
//--QUERY FOR ALL PATIENTS REPORT--------
$totalpaseg=$this->db->select('totalpaseg')->where('id2',$row1->idfacc)->get('factura')->row('totalpaseg');
//------------------------------------------
...
$money_device = $this->db->select('money_device')->where('id_factura', $row1->idfacc)->get('factura_modalidad_pago')->row('money_device');
if($money_device){
$money_device=$money_device;	
}else{
$money_device='RD$';	
}
if($report_type=='one-patient'){
?>
TOTAL RECLAMADO : <b><?=$money_device?> <?=$sbt?></b>&nbsp;&nbsp;&nbsp;
ASEGURADORA PAGARA : <b><?=$money_device?> <?=$tps?></b>&nbsp;&nbsp;&nbsp;
 EL PACIENTE PAGARA : <b><?=$money_device?>  <?=$tpat?></b>&nbsp;&nbsp;&nbsp;
<?php }else{ ?>
...
<br/>-->
<a style="color:red;font-size:11px" href="<?=site_url()?>signature/create_patient_signature/<?=$row1->paciente?>/<?=$row1->idfacc?>">Crear La Firma Del Paciente</a>
