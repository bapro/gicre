...
$name=$this->session->userdata['farmacia_name'];
$perfil=$this->session->userdata['farmacia_perfil'];
$id_farmacia=$this->db->select('id_farmacia')->where('id_user',$id_usr)->get('user_farmacia')->row('id_farmacia');
$farmacia=$this->db->select('noma')->where('id',$id_farmacia)->get('drug_stores')->row('noma');
$sqlc ="SELECT * FROM h_c_indicaciones_medicales WHERE despachada=2";
$queryc=$this->db->query($sqlc);

$sqld ="SELECT * FROM h_c_indicaciones_medicales  WHERE farma=$id_farmacia";
$querydes= $this->db->query($sqld);
...
<li class="dropdown">
<!--<li><a href="<?php echo site_url('farmacia/index');?>">Buscar Recetas</a></li>-->
<li><a href="<?php echo site_url('farmacia/despachadas');?>"><?=$querydes->num_rows()?> Recetas Despachadas</a></li>
</li>
...
<li class="dropdown" style="font-size:10px" >
  ...
  <img src="<?= base_url();?>assets/img/user.png" style="width:25px;border-radius:20px" alt=""/> <?= $name; ?> : <?=$farmacia; ?> 
...
<li><a href="<?php echo site_url('login/farmacia_logout');?>" ><i class="fa fa-sign-out"></i>  Cerrar Session</a></li>
...
window.location ="<?php echo base_url(); ?>farmacia/search_code1?code="+code; 	
