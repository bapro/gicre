...
class Hosp_examen_sitema extends CI_Controller
{
	  ...
	  function saveExamenSistema()
        {
            if ($this->input->post('form_inputs') > 0) {
                ...
                if ($this->input->post('id') == 0) {
                    $this->hospitalization_emgergency->insert("hosp_examen_sistema", $data);
					echo '<i class="bi bi-check  text-green" ></i>';
                } else {
                    $this->hospitalization_emgergency->where('id', $this->input->post('id'));
                    $this->hospitalization_emgergency->update('hosp_examen_sistema', $data);
                    echo '<i class="bi bi-check  text-green" ></i>';
                }
            }else{
    ...
    $sql="SELECT * FROM hosp_examen_sistema WHERE historial_id=$this->ID_PATIENT ORDER BY id DESC";
	...
	$query_sis=$this->hospitalization_emgergency->query("SELECT * FROM  hosp_examen_sistema WHERE id=$page");
	$data['query_sis']= $query_sis->result();
	$this->load->view('hospitalization/clinical-history/examen-sistema/form', $data);
