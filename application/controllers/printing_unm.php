...
class Printing_unm extends CI_Controller
{
    public function __construct()
    {
        ...
        $this
            ->load
            ->model('model_auditoria_medica');
        $this
            ->load
            ->model('model_medico');
        $this->padron_database = $this
            ->load
            ->database('padron', true);
    }
    ...
    public function search_code_estudios_result()
    {
        ...
        $estudios = $this
            ->model_admin
            ->print_estudio_patient_search($code);
        $this->data['estudios'] = $estudios;
        foreach ($estudios as $row) $this->data['id_historial'] = $row->historial_id;
        $this->data['id_doc'] = $row->operator;
        $this->data['title'] = "ESTUDIOS";
        $paciente = $this
            ->db
            ->select('photo,ced1,ced2,ced3')
            ->where('id_p_a', $row->historial_id)
        ...
        if ($paciente['photo'] == "padron")
        {
            $photo = $this
                ->padron_database
                ->select('IMAGEN')
                ->where('MUN_CED', $paciente['ced1'])->where('SEQ_CED', $paciente['ced2'])->where('VER_CED', $paciente['ced3'])->get('fotos')
                ->row('IMAGEN');
            if ($photo)
            {
        ...
        $html = $this
            ->load
            ->view('admin/print/header-print-hist_ind', $this->data, true);
        $html2 = $this
            ->load
            ->view('admin/print/estudios', $this->data, true);
        $mpdf->WriteHTML($html);
    ...
    public function hosp_orden_medica($user_id, $patient_id, $centro, $id_hosp)
    {
        ...
        $this
            ->db
            ->insert("hosp_orden_medica_sala", $save);
        $last_id = $this
            ->db
            ->insert_id();
        $data = array(
        ...
        $this
            ->db
            ->update("hosp_orden_medica_recetas", $data);
        $where2 = array(
            'historial_id' => $patient_id,
            'operator' => $user_id,
            'printing' => 2
        ...
        $this
            ->db
            ->update("hosp_orden_medica_estudios", $data);
        $where3 = array(
        ...
        $this
            ->db
            ->update("hosp_orden_medcia_lab", $data);
        $this
            ->session
            ->set_userdata('user_id_emg', $user_id);
        $this
            ->session
            ->set_userdata('patient_id_emg', $patient_id);
        $this
            ->session
            ->set_userdata('centro_emg', $centro);
        $this
            ->session
            ->set_userdata('id_sala', $last_id);
        $this
            ->session
            ->set_userdata('id_hosp', $id_hosp);
    ...
    public function hosp_orden_medica_($user_id, $patient_id, $id_sala, $id_hosp)
    {
        $this
            ->session
            ->set_userdata('user_id_om', $user_id);
        $this
            ->session
            ->set_userdata('patient_id', $patient_id);
        $this
            ->session
            ->set_userdata('id_sala', $id_sala);
        $this
            ->session
            ->set_userdata('id_hosp', $id_hosp);
    ...
    public function hosp_print_orden_medica()
    {
        ...
        $this->data['id_hosp'] = $this
            ->session
            ->userdata['id_hosp'];
        $paciente = $this
            ->db
            ->select('photo,ced1,ced2,ced3,date_nacer')
        ...
        $html = $this
            ->load
            ->view('admin/print/header-print-hospitalizacion', $this->data, true);
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_orden_medica', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_hosp_orden_medica()
    {
        ...
        $patient_id = $this
            ->session
            ->userdata['patient_id_emg'];
        $this->data['centro'] = $this
            ->session
            ->userdata['centro_emg'];
        $this->data['patient_id'] = $patient_id;
        $this->data['user_id'] = $user_id;
        $this->data['id_sala'] = $this
            ->session
            ->userdata['id_sala'];
        $this->data['id_hosp'] = $this
            ->session
        ...
        $this->data['area'] = $this
            ->db
            ->select('title_area')
            ->where('id_ar', $doc['area'])->get('areas')
            ->row('title_area');
        $this->data['id_historial'] = $patient_id;
    ...
    public function search_code()
    {
        ...
        $farma = $this
            ->db
            ->select('historial_id,operator')
            ->where('encrypt_recetas', $code)->get('h_c_indicaciones_medicales')
            ->row_array();
        $this->data['id_historial'] = $farma['historial_id'];
        $print_recetas = $this
            ->model_admin
            ->print_recetas_for_patient($code);
        $this->data['print_recetas'] = $print_recetas;
        foreach ($print_recetas as $rowid) $this->data['id_doc'] = $rowid->operator;
        ...
        if ($print_recetas != NULL)
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $farma['historial_id'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
            ...
            $html2 = $this
                ->load
                ->view('admin/print/recetas', $this->data, true);
            $mpdf->WriteHTML($html);
            $mpdf->WriteHTML($html2);
            $mpdf->Output();
        }
    ...
    public function print_ant_gnrl($id1, $id2)
    {
        $this
            ->session
            ->set_userdata('historial_id', $id1);
        $this
            ->session
            ->set_userdata('val_gnrl', $id2);
    ...
    public function emergencia_orden_medica($user_id, $patient_id, $centro, $enviado_a)
    {
        ...
        $this
            ->db
            ->update("orden_medica_recetas", $data);
        $where2 = array(
            'historial_id' => $patient_id,
            'operator' => $user_id,
            'printing' => 2
        ...
        $this
            ->db
            ->update("orden_medica_estudios", $data);
        $where3 = array(
            'id_patient' => $patient_id,
            'id_user' => $user_id,
            'printing' => 2
        ...
        $this
            ->session
            ->set_userdata('enviado_a', $enviado_a);
    ...
    public function print_emergencia_orden_medica()
    {
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_emergencia_orden_medica', $this->data, true);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print_orden_medica()
    {
        ...
        if ($id_sala == 0)
        {
            $sala = $this
                ->db
                ->select('idsala')
                ->where("id_user", $user_id)->where("id_historial", $patient_id)->order_by('idsala', 'desc')
                ->limit(1)
    ...
    public function print_hoy_cita1($id1, $perfil)
    {
        $this
            ->session
            ->set_userdata('id_user', $id1);
        $this
            ->session
            ->set_userdata('perfil', $perfil);
    ...
    public function print_hoy_cita()
    {
        ...
        else
        {
            $this->data['appointments'] = $this
                ->model_medico
                ->getAsMedicoAp($id_user);
        }
        $html = $this
            ->load
            ->view('admin/print/print_hoy_cita', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_hoy_cita2($id1, $perfil, $date1, $date2, $centro)
    {
        ...
        $this
            ->session
            ->set_userdata('date1', $date1);
        $this
            ->session
            ->set_userdata('date2', $date2);
        $this
            ->session
            ->set_userdata('centro', $centro);
    ...
    public function print_cita_date_range()
    {
        ...
        if ($perfil == "Admin")
        {
            $this->data['appointments'] = $this
                ->model_admin
                ->get_centro_medico_datepicker($datas);
        }
        else
        {
            $val = array(
            ...
            $this->data['appointments'] = $this
                ->model_medico
                ->get_centro_medico_datepicker($val);
        }
        $html = $this
            ->load
            ->view('admin/print/print_hoy_cita', $this->data, true);
    ...
    public function print002()
    {
        ...
        $updated_by = $this
            ->db
            ->select('update_by,date_modif,id_user')
            ->where('historial_id', $historial_id)->get('h_c_marque_positivo')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
            ->where('id_user', $updated_by['update_by'])->get('users')
            ->row('name');
        $perfil = $this
            ->db
            ->select('perfil')
            ->where('id_user', $updated_by['id_user'])->get('users')
            ->row('perfil');
        if ($perfil == "Admin")
        {
        ...
        $this->data['row_ant'] = $this
            ->model_admin
            ->showAnte($historial_id);
        $this->data['desa'] = $this
            ->model_admin
            ->showDesarollo($historial_id);
        $this->data['AntOtros'] = $this
            ->model_admin
            ->GetAntOtros($historial_id);
        $this->data['medicamentos'] = $this
            ->model_admin
            ->medicamentos();
        $this->data['selectOne'] = $this
            ->model_admin
            ->selectOne();
        $this->data['selectTwo'] = $this
            ->model_admin
            ->selectTwo();
        $this->data['HABITOS'] = $this
            ->model_admin
        ...
        $html = $this
            ->load
            ->view('admin/print/header-print-hist', $this->data, true);
        $html2 = $this
            ->load
            ->view('admin/print/print_ant_gnrl', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function signaturePatient()
    {
        $this
            ->session
            ->set_userdata('patient_firma', $this
            ->uri
            ->segment(3));
        $this
            ->session
            ->set_userdata('id_fac', $this
            ->uri
            ->segment(4));
    ...
    public function patient_sign_in()
    {
        ...
        $pat = $this
            ->db
            ->select('nombre,photo,ced1,ced2,ced3')
            ->where('id_p_a', $this
            ->session
            ->userdata['patient_firma'])
            ->get('patients_appointments')
            ->row_array();
        ...
        if ($pat['photo'] == "padron")
        {
            $photo = $this
                ->padron_database
                ->select('IMAGEN')
                ->where('MUN_CED', $pat['ced1'])->where('SEQ_CED', $pat['ced2'])->where('VER_CED', $pat['ced3'])->get('fotos')
                ->row('IMAGEN');
            if ($photo)
            {
                $imgpat = '<img  style="width:70px;"  src="data:image/jpeg;base64,' . base64_encode($photo) . '"  />';
        ...
        $this
            ->load
            ->view('admin/print/signature-patient', $data);
    }
    public function sendLinkToPatientForSingning()
    {
        ...
        $pat = $this
            ->db
            ->select('nombre,photo,ced1,ced2,ced3')
            ->where('id_p_a', $patient_firma)->get('patients_appointments')
            ->row_array();
        $data['patiente'] = $pat['nombre'];
        if ($pat['photo'] == "padron")
        {
        ...
        $this
            ->load
            ->view('admin/print/signature-patient-link', $data);
    }
    public function signature()
    {
        $this
            ->session
            ->set_userdata('from', $this
            ->uri
            ->segment(3));
        $this
            ->session
            ->set_userdata('is_doc', $this
            ->uri
            ->segment(4));
        $this
            ->session
            ->set_userdata('id_imp', $this
            ->uri
            ->segment(5));
        $this
            ->session
            ->set_userdata('who', $this
            ->uri
            ->segment(6));
        $this
            ->session
            ->set_userdata('patient_cont', $this
            ->uri
            ->segment(7));
    ...
    public function sign_in()
    {
        ...
        $data['who'] = $this
            ->session
            ->userdata['who'];
        $data['patient_cont'] = $this
            ->session
            ->userdata['patient_cont'];
        $data['doc'] = $this
            ->db
            ->select('name')
            ->where('id_user', $this
        ...
        $this
            ->load
            ->view('admin/print/signature', $data);
    }
    ...
    public function print_enf_act()
    {
        $this
            ->session
            ->set_userdata('idenf', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_enf', $this
            ->input
            ->get('val'));
    ...
    public function print003()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,user_id,updated_time,updated_by')
            ->where('id_enf', $id)->get('h_c_sinopsis')
            ->row_array();
        $this->data['id_doc'] = $val['updated_by'];
        $author = $this
            ->db
            ->select('name')
            ->where('id_user', $val['updated_by'])->get('users')
            ->row('name');
        $this->data['id_historial'] = $val['historial_id'];
        $this->data['print_enf_act'] = $this
            ->model_admin
            ->show_enfermedad($id);
        ...
        $perfil = $this
            ->db
            ->select('perfil')
            ->where('id_user', $val['user_id'])->get('users')
            ->row('perfil');
        if ($perfil == "Admin")
        {
            $this->data['updated_by'] = $author;
        ...
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $val['historial_id'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_enf_act', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_conc_d()
    {
        $this
            ->session
            ->set_userdata('id', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_conc', $this
            ->input
            ->get('val'));
        redirect('printings/print004');
    }
    public function diag_p_vert($id, $idPat, $foto, $pos, $originFac, $centroTimbrado)
    {
        $this
            ->session
            ->set_userdata('id', $id);
        $this
            ->session
            ->set_userdata('idPat', $idPat);
        $this
            ->session
            ->set_userdata('val_conc', $foto);
        $this
            ->session
            ->set_userdata('pos', $pos);
        $this
            ->session
            ->set_userdata('originFac', $originFac);
        $this
            ->session
            ->set_userdata('centroTimbrado', $centroTimbrado);
    ...
    public function print004pvert()
    {
        ...
        if ($originFac == 0)
        {
            $where = "WHERE id_cd=$id";
            $userInfo = $this
                ->db
                ->select('id_cd,user,patient,time_created')
                ->where('id_cd', $id)->get('h_c_procedimiento_tarif')
                ->row_array();
        }
        else
        {
            $where = "WHERE autoNomber=$id && id_cd=0";
            $userInfo = $this
                ->db
                ->select('id_cd,user,patient,time_created')
                ->where('autoNomber', $id)->where('id_cd', 0)
                ->get('h_c_procedimiento_tarif')
                ->row_array();
        }
        $sqltp = "SELECT * FROM h_c_procedimiento_tarif $where";
        $this->data['tarif_proced'] = $this
            ->db
            ->query($sqltp);
        $this->data['id'] = $id;
        $this->data['id_cd'] = $userInfo['id_cd'];
        $this->data['id_doc'] = $userInfo['user'];
        $val = $this
            ->db
            ->select('historial_id,id_user,updated_time,updated_by,procedimiento,otros_diagnos,current_day')
            ->where('id_cdia', $id)->get('h_c_conclusion_diagno')
            ->row_array();
        $this->data['id_historial'] = $userInfo['patient'];
        $this->data['procedimiento'] = $val['procedimiento'];
        $this->data['otros_diagnos'] = $val['otros_diagnos'];
        $this->data['signopsis'] = $this
            ->db
            ->select('signopsis')
            ->where('inserted_by', $userInfo['user'])->where('historial_id', $userInfo['patient'])->where('filter_date', $val['current_day'])->get('h_c_sinopsis')
            ->row('signopsis');
        $date_modif = date("d-m-Y H:i:s", strtotime($userInfo['time_created']));
        $doc = $this
            ->db
            ->select('name,exequatur,area')
            ->where('id_user', $userInfo['user'])->get('users')
            ->row_array();
        $execuatur = $doc['exequatur'];
        $docName = $doc['name'];
        $area = $this
            ...
            ->where('id_ar', $doc['area'])->get('areas')
            ->row('title_area');
        $this->data['docInfo'] = "Doc(a) $docName, $area, Exeq. $execuatur, <span style='color:red'>$date_modif</span> ";
        $this->data['print_cond'] = $this
            ->model_admin
            ->show_con_by_id($id, 0);
        ...
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $userInfo['patient'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
                ...
                if ($photo)
                {
                    $imgpat = '<img src="data:image/jpeg;base64,' . base64_encode($photo) . '"style="width:70px">';
                }
                else
                {
                    $imgpat = '<img src="' . base_url() . '/assets/img/user.png"style="width:70px">';
                }
            }
            else if ($paciente['photo'] == "")
            {
            ...
            {
                $imgpat = '<img src="' . base_url() . '/assets/img/patients-pictures/' . $paciente['photo'] . '"style="width:70px;height:70px;object-position:50% 50%">';
            }
            $this->data['display'] = "<td style='width:40px'>$imgpat</td>";
        }
        $html = $this
            ->load
            ->view('admin/print/header-centro-timbrado', $this->data, true);
        $html2 = $this
            ->load
            ->view('admin/print/print_diag_p_f', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print004()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,id_user,updated_time,updated_by,procedimiento,otros_diagnos')
            ->where('id_cdia', $id)->get('h_c_conclusion_diagno')
            ->row_array();
        $author = $this
            ->db
        ...
        $perfil = $this
            ->db
            ->select('perfil')
            ->where('id_user', $val['id_user'])->get('users')
            ->row('perfil');
        if ($perfil == "Admin")
        {
            $this->data['updated_by'] = $author;
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_conc_d', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_rehab()
    {
        $this
            ->session
            ->set_userdata('idrehab', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_rehab', $this
            ->input
            ->get('val'));
    ...
    public function print005()
    {
        ...
        $val = $this
            ->db
            ->select('id_historial,updated_time,updated_by,id_user')
            ->where('id_rehab', $id)->get('h_c_rehabilitacion')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $this->data['showRehabilidad'] = $this
            ->model_admin
            ->showRehabById($id);
        $this->data['title'] = "REHABILITACION";
        if ($this
            ->session
            ->userdata['val_rehab'] == 0)
        {
            $this->data['display'] = "";
        }
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $val['id_historial'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
        ...
        $html2 = $this
            ->load
            ->view('admin/print/rehab', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_exa_f()
    {
        $this
            ->session
            ->set_userdata('idexf', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_exf', $this
            ->input
            ->get('val'));
    ...
    public function print006()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,updated_by,updated_time,id_user')
            ->where('id_sig', $id)->get('h_c_examen_fisico')
            ->row_array();
        $this->data['id_doc'] = $val['updated_by'];
        $author = $this
            ->db
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_exa_f', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print0009()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,updated_by,updated_time,user_id,id_sig')
            ->where('idot', $id)->get('h_c_examen_fis_otorrino')
            ->row_array();
        $id_ex_f = $val['id_sig'];
        $author = $this
            ->db
        ...
        $this->data['show_exam_ot_by_id'] = $this
            ->db
            ->query($sql);
        $this->data['ExamFisById'] = $this
            ->model_admin
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_exa_fotorrino', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_exam_sis()
    {
        $this
            ->session
            ->set_userdata('idsis', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_sis', $this
            ->input
            ->get('val'));
    ...
    public function print_alerg()
    {
        $this
            ->session
            ->set_userdata('idal', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_al', $this
            ->input
            ->get('val'));
    ...
    public function print_alerg01()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,updated_by,updated_time')
            ->where('id', $id)->get('h_c_ant_alergista')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_alergista', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print007()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,updated_by,updated_time,id_user')
            ->where('id_exs', $id)->get('h_c_examen_sistema')
            ->row_array();
        $this->data['id_doc'] = $val['updated_by'];
        $author = $this
            ->db
        ...
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $this->data['show_examsis_by_id'] = $this
            ->model_admin
            ->show_examsis_by_id($id);
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_exam_sis', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_ssr()
    {
        $this
            ->session
            ->set_userdata('idssr', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_ssr', $this
            ->input
            ->get('val'));
    ...
    public function print008()
    {
        ...
        $val = $this
            ->db
            ->select('hist_id,updated_by,update_time,id_user')
            ->where('idssr', $id)->get('h_c_ant_ssr')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $val['hist_id'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_ssr', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_ant_obs()
    {
        $this
            ->session
            ->set_userdata('idobs', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_obs', $this
            ->input
            ->get('val'));
    ...
    public function print009()
    {
        ...
        $val = $this
            ->db
            ->select('hist_id,updated_by,updated_time,id_user')
            ->where('idobs', $id)->get('h_c_ante_obs1')
            ->row_array();
        $this->data['id_doc'] = $val['updated_by'];
        $author = $this
            ->db
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_ant_obs', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_ant_pedia($id, $val)
    {
        $this
            ->session
            ->set_userdata('id_pedia', $id);
        $this
            ->session
            ->set_userdata('val_pedia', $val);
    ...
    public function print0010()
    {
        ...
        $val = $this
            ->db
            ->select('hist_id,updated_time,updated_by,id_user')
            ->where('id', $id)->get('h_c_ant_pedia')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $this->data['vacuna'] = $this
            ->model_admin
            ->getVacunaData($val['hist_id']);
        $this->data['pediaid'] = $this
            ->model_admin
            ->getPediaId($id);
        $this->data['title'] = "ANTECEDENTE PEDIATRICO";
        if ($this
            ->session
            ->userdata['val_pedia'] == 0)
        {
            $this->data['display'] = "";
        }
        else
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_ant_pedia', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_cont_p()
    {
        $this
            ->session
            ->set_userdata('idcpn', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_ctp', $this
            ->input
            ->get('val'));
    ...
    public function print0011()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,updated_time,updated_by,id_user')
            ->where('id_c1', $id)->get('h_c_control_prenatal1')
            ->row_array();
        $this->data['id_doc'] = $val['updated_by'];
        $author = $this
            ->db
        ...
        $this->data['showSelectContP1'] = $this
            ->model_admin
            ->showSelectContP1($id);
        $this->data['showSelectContP2'] = $this
            ->model_admin
            ->showSelectContP2($id);
        $this->data['showSelectContP3'] = $this
            ->model_admin
            ->showSelectContP3($id);
        $this->data['showSelectContP4'] = $this
            ->model_admin
            ->showSelectContP4($id);
        $this->data['showSelectContP5'] = $this
            ->model_admin
            ->showSelectContP5($id);
        $this->data['showSelectContP6'] = $this
            ->model_admin
            ->showSelectContP6($id);
        $this->data['showSelectContP7'] = $this
            ->model_admin
            ->showSelectContP7($id);
        $this->data['showSelectContP8'] = $this
            ->model_admin
            ->showSelectContP8($id);
        $this->data['showSelectContP9'] = $this
            ->model_admin
            ->showSelectContP9($id);
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_cont_p', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function invoice_ars_claim_()
    {
        $last_id = $this
            ->db
            ->select('id_ncf')
            ->order_by('id_ncf', 'desc')
            ->limit(1)
            ->get('ncf')
            ->row('id_ncf');
        $this
            ->session
            ->set_userdata('last_id', $last_id);
    ...
    public function print0012()
    {
        ...
        $this->data['invoice'] = $this
            ->model_admin
            ->getNewinvoice($last_id);
        $this->data['nota_ncf'] = $this
            ->model_admin
            ->getNcf($last_id);
        $html = $this
            ->load
            ->view('admin/print/print_invoice_ars', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function medical_insurance_audit_profile_print()
    {
        ...
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $this->data['desde'] = $this
            ->uri
            ->segment(3);
        $this->data['hasta'] = $this
            ->uri
            ->segment(4);
        $this->data['medico'] = $this
            ->uri
            ->segment(5);
        $this->data['id_seguro'] = $this
            ->uri
            ->segment(7);
        if ($this
            ->uri
        ...
        {
            $results = $this
                ->model_auditoria_medica
                ->show_patient_arc_report_ars($data2);
            $this->data['patient_rows'] = $results;
            $this->data['count'] = count($results);
        }
        $html = $this
            ->load
            ->view('admin/print/medical_insurance_audit_profile', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_indicaciones($historial_id, $position, $ifphoto, $col, $table, $centroId)
    {
        $this
            ->session
            ->set_userdata('id', $historial_id);
        $this
            ->session
            ->set_userdata('position', $position);
        $this
            ->session
            ->set_userdata('ifphoto', $ifphoto);
        $this
            ->session
            ->set_userdata('col', $col);
        $this
            ->session
            ->set_userdata('table', $table);
        $this
            ->session
            ->set_userdata('centroId', $centroId);
    ...
    public function print016()
    {
        ...
        $table = $this
            ->session
            ->userdata['table'];
        $this->data['centroId'] = $this
            ->session
            ->userdata['centroId'];
        $query_rows = $this
            ->model_admin
            ->print_indicaciones($historial_id, $col, $table);
        if ($query_rows != NULL)
        {
            $this->data['query_rows'] = $query_rows;
            foreach ($query_rows as $rowid) if ($position == 'vert')
            {
                $mpdf = new \Mpdf\Mpdf(['tempDir' => __DIR__ . '/custom/temp/dir/path', 'format' => 'A5']);
            }
            else
            ...
            $html2 = $this
                ->load
                ->view('admin/print/indicaciones', $this->data, true);
            $mpdf->WriteHTML($html);
            $mpdf->WriteHTML($html2);
            $mpdf->Output();
        }
    ...
    public function emMed($id_patient, $opid, $fecha, $valf)
    {
        $this
            ->session
            ->set_userdata('id_patient', $id_patient);
        $this
            ->session
            ->set_userdata('opid', $opid);
        $this
            ->session
            ->set_userdata('fecha', $fecha);
        $this
            ->session
            ->set_userdata('valf', $valf);
    ...
    public function emMedP01()
    {
        ...
        if ($print_recetas->result() != NULL)
        {
            ...
            $html2 = $this
                ->load
                ->view('admin/print/print_emergencia_medicamentos', $this->data, true);
            $mpdf->WriteHTML($html);
            $mpdf->WriteHTML($html2);
            $mpdf->Output();
        }
    ...
    public function printemed()
    {
        ...
        $print_recetas = $this
            ->model_admin
            ->print_emergencia_medicamentos($historial_id);
        $this->data['print_recetas'] = $print_recetas;
        $data = array(
            'printed' => 0
        );
        ...
        $this
            ->db
            ->update('emergency_medicaments', $data);
        $this->data['user_id'] = $this
            ->uri
            ->segment(4);
        $this->data['title'] = "MEDICAMENTOS";
        if ($print_recetas != NULL)
        {
            $html = $this
                ->load
                ->view('admin/print/header-print-hist_ind_horiz', $this->data, true);
            $html2 = $this
                ->load
                ->view('admin/print/print_emergencia_medicamentos', $this->data, true);
            $mpdf->WriteHTML($html);
    ...
    public function printh016()
    {
        ...
        $print_recetas = $this
            ->model_admin
            ->print_recetas($historial_id);
        $this->data['print_recetas'] = $print_recetas;
        foreach ($print_recetas as $rowid) $this->data['id_doc'] = $rowid->updated_by;
        $updt = array(
            'singular_id' => 0
        );
        $this
            ->model_admin
            ->UpdateSingularId($historial_id, $updt);
        $this->data['user_id'] = $this
            ->uri
            ->segment(4);
        $this->data['title'] = "RECETAS";
    ...
    public function print_es_if_foto_oriz($historial_id, $id, $area, $user)
    {
        ...
        $this
            ->load
            ->view('admin/print/print_es_if_foto_oriz', $data);
    }
    public function print_if_foto_oriz($historial_id, $id, $area, $user)
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto_oriz', $data);
    }
    public function print_if_foto($historial_id, $id, $area, $user)
    {
        $this
            ->session
            ->set_userdata('historial_id', $historial_id);
        $this
            ->session
            ->set_userdata('id', $id);
        $this
            ->session
            ->set_userdata('area', $area);
        $this
            ->session
    ...
    public function print_if_foto_em($id1, $id2, $id3, $id4, $id5, $id6)
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto_em', $data);
    }
    public function print_emergency($id1, $id2, $id3, $id4, $id5, $id6, $val)
    {
        if ($id5 == 'ev')
        {
            $this
                ->session
                ->set_userdata('id_ev', $id1);
            $this
                ->session
                ->set_userdata('patient_ev', $id2);
            $this
                ->session
                ->set_userdata('centro_medico_ev', $id4);
            $this
                ->session
                ->set_userdata('doc_ev', $id3);
            $this
                ->session
                ->set_userdata('val_ev', $val);
            $this
                ->session
                ->set_userdata('enviado1', $id6);
        ...
        else if ($id5 == 'ni')
        {
            $this
                ->session
                ->set_userdata('id_ni', $id1);
            $this
                ->session
                ->set_userdata('patient_ni', $id2);
            $this
                ->session
                ->set_userdata('centro_medico_ni', $id4);
            $this
                ->session
                ->set_userdata('doc_ni', $id3);
            $this
                ->session
                ->set_userdata('val_ni', $val);
            $this
                ->session
                ->set_userdata('enviado2', $id6);
    ...
    public function print_if_foto2()
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto', $data);
    }
    public function print_if_foto_c1($id1, $id2, $id3, $id4, $id5)
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto_rep_gen_horiz', $data);
    }
    public function print_if_foto_c($id1, $id2, $id3, $id4, $id5)
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto_c', $data);
    }
    public function print_cirugia_report($id1, $id2, $id3, $id4, $id5, $val)
    {
        if ($id5 == 'g')
        {
            if ($id1 == 'c_t_0')
            {
                $id_insert = $this
                    ->db
                    ->select('id')
                    ->where('id_patient', $id2)->where('user', $id3)->order_by('id', 'desc')
                    ->limit(1)
                    ->get('hc_cirugia_toracica')
                    ->row('id');
            }
            else
            {
            ...
            $this
                ->session
                ->set_userdata('id_insert', $id_insert);
            $this
                ->session
                ->set_userdata('id_patient', $id2);
            $this
                ->session
                ->set_userdata('centro_medico_c', $id4);
            $this
                ->session
                ->set_userdata('id_doc', $id3);
            $this
                ->session
                ->set_userdata('val_1', $val);
        ...
        else if ($id5 == 'q')
        {
            if ($id1 == 'c_t_0')
            {
                $id_insert = $this
                    ->db
                    ->select('id')
                    ->where('id_patient', $id2)->where('id_user', $id3)->order_by('id', 'desc')
                    ->limit(1)
                    ->get('hc_quirurgica')
                    ->row('id');
            }
        ...
        else if ($id5 == 'ipps')
        {
            if ($id1 == 'new_ipss')
            {
                $id_insert = $this
                    ->db
                    ->select('id')
                    ->where('historial_id', $id2)->where('id_user', $id3)->order_by('id', 'desc')
                    ->limit(1)
                    ->get('h_c_ipss')
                    ->row('id');
            }
        ...
        else if ($id5 == 'card')
        {
            if ($id1 == 'new_card')
            {
                $id_insert = $this
                    ->db
                    ->select('id')
                    ->where('patient', $id2)->where('id_user', $id3)->order_by('id', 'desc')
                    ->limit(1)
                    ->get('hc_evaluacion_car_vas')
                    ->row('id');
            }
            else
            {
        ...
        else
        {
            ...
            $this
                ->session
                ->set_userdata('id_insertr', $id_insert);
            $this
                ->session
                ->set_userdata('id_patientr', $id2);
            $this
                ->session
                ->set_userdata('centro_medico_cr', $id4);
            $this
                ->session
                ->set_userdata('id_docr', $id3);
            $this
                ->session
                ->set_userdata('val_1r', $val);
    ...
    public function print_cirugia_general_report_horiz_()
    {
        ...
        $id_patient = $this
            ->session
            ->userdata['id_patientr'];
        $this->data['centroId'] = $this
            ->session
            ->userdata['centro_medico_cr'];
        $id_doc = $this
            ->session
            ->userdata['id_docr'];
        ...
        $doc = $this
            ->db
            ->select('name,exequatur')
            ->where('id_user', $id_doc)->get('users')
            ->row_array();
        ...
        $cirugia_toracico = $this
            ->db
            ->query($sql);
        foreach ($cirugia_toracico->result() as $row) $this->data['title'] = "$row->reporte";
        $val_1 = $this
            ->session
            ->userdata['val_1r'];
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        ...
        $html = $this
            ->load
            ->view('admin/print/header-print-hist_ind', $this->data, true);
        $this->data['cirugia_toracico'] = $this
            ->db
            ->query($sql);
        $html2 = $this
            ->load
            ->view('admin/print/cirugia_toracia_report', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_cardio()
    {
        ...
        $this->data['ipss_data'] = $this
            ->db
            ->query($sql);
        $html2 = $this
            ->load
            ->view('admin/print/print_cardio', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_ipss()
    {
        ...
        $html = $this
            ->load
            ->view('admin/print/header-centro', $this->data, true);
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_ipss', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_new_inter()
    {
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_new_inter', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_ev_nota()
    {
        ...
        $html2 = $this
            ->load
            ->view('admin/print/print_ev_nota', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print_cirugia()
    {
        ...
        $html2 = $this
            ->load
            ->view('admin/print/cirugia_toracia', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_quirurgica()
    {
        ...
        else
        {
            ...
            $this->data['display'] = "<td  class='td-head'>$imgpat</td>";
        }
        $html2 = $this
            ->load
            ->view('admin/print/print_quirurgica', $this->data, true);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print_if_foto1()
    {
        ...
        $this
            ->load
            ->view('admin/print/print_if_foto_', $data);
    }
    public function print_estudios($historial_id, $id, $val, $pos, $centroId)
    {
        $this
            ->session
            ->set_userdata('id_p', $historial_id);
        $this
            ->session
            ->set_userdata('id_es', $id);
        $this
            ->session
            ->set_userdata('val', $val);
        $this
            ->session
            ->set_userdata('pos', $pos);
        $this
    ...
    public function print017()
    {
        ...
        $estudios = $this
            ...
            ->print_estudio($id_es);
        $this->data['estudios'] = $estudios;
        foreach ($estudios as $rowid) $this->data['id_doc'] = $rowid->updated_by;
        if ($this
            ->session
            ->userdata['val'] == 0)
        {
        ...
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $id_p)->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
    ...
    public function print_estudios_horiz($id_p, $id_es, $area, $user_id, $valhes)
    {
        $this
            ->session
            ->set_userdata('id_p', $id_p);
        $this
            ->session
            ->set_userdata('id_es', $id_es);
        $this
            ->session
            ->set_userdata('area', $area);
        $this
    ...
    public function print_labo()
    {
        $this
            ->session
            ->set_userdata('id_p', $this
            ->uri
            ->segment(3));
        $this
            ->session
            ->set_userdata('area', $this
            ->uri
            ->segment(4));
        $this
            ->session
            ->set_userdata('user_id', $this
            ->uri
            ->segment(5));
        $this
            ->session
            ->set_userdata('val_lab', $this
            ->uri
            ->segment(6));
        redirect('printings/print018');
    }
    public function print_laboratorios($historial_id, $position, $ifphoto, $col)
    {
        $this
            ->session
            ->set_userdata('id', $historial_id);
    ...
    public function print018()
    {
            ...
            ->segment(4) == 0)
        {
            $IndicacionesLab = $this
                ->model_admin
                ->PrintlaboratoriosAdmin($print);
        }
        else
        {
            $IndicacionesLab = $this
                ->model_admin
                ->Printlaboratorios($print);
        }
        $this->data['IndicacionesLab'] = $IndicacionesLab;
        foreach ($IndicacionesLab as $rowid) $this->data['id_doc'] = $rowid->updated_by;
        $updt = array(
            'printing' => 0
        );
        $this
            ->model_admin
            ->updateIndicacionesLab($updt, $id_p);
        $this->data['title'] = "LABORATORIOS";
        if ($IndicacionesLab != NULL)
        {
            ...
            $html2 = $this
                ->load
                ->view('admin/print/lab', $this->data, true);
            $mpdf->WriteHTML($html);
            $mpdf->WriteHTML($html2);
            $mpdf->Output();
        }
    ...
    public function oftal()
    {
        $this
            ->session
            ->set_userdata('idoftal', $this
            ->input
            ->get('historial_id'));
        $this
            ->session
            ->set_userdata('val_oftal', $this
            ->input
            ->get('val'));
    ...
    public function print015000h()
    {
        ...
        $val = $this
            ->db
            ->select('id_hist,id_user,inserted')
            ->where('id', $id)->get('h_c_of_refracion')
            ->row_array();
        $this->data['author'] = $this
            ->db
            ->select('name')
            ->where('id_user', $val['id_user'])->get('users')
            ->row('name');
        ...
        else
        {
            $paciente = $this
                ->db
                ->select('photo,ced1,ced2,ced3')
                ->where('id_p_a', $val['id_hist'])->get('patients_appointments')
                ->row_array();
            if ($paciente['photo'] == "padron")
            {
                $photo = $this
        ...
        $html = $this
            ->load
            ->view('admin/print/header-refraccion', $this->data, true);
        $this->data['docid'] = $val['id_user'];
        $html2 = $this
            ->load
            ->view('admin/print/oftal-ref', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print0150()
    {
        ...
        $val = $this
            ->db
            ->select('id_historial,user_id,updated_time,updated_by')
            ->where('id_oftal', $id)->get('h_c_oftalmologia')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        $html2 = $this
            ->load
            ->view('admin/print/oftal-ref1', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print015()
    {
        ...
        $html2 = $this
            ->load
            ->view('admin/print/oftal', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    ...
    public function print014()
    {
        ...
        $val = $this
            ->db
            ->select('historial_id,user_id,updated_time,updated_by')
            ->where('id_derma', $id)->get('h_c_dermatologo')
            ->row_array();
        $author = $this
            ->db
            ->select('name')
        ...
        $html2 = $this
            ->load
            ->view('admin/print/dermatologo', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->WriteHTML($html2);
        $mpdf->Output();
    }
    public function print_billing()
    {
        $this
            ->session
            ->set_userdata('idfacc', $this
            ->uri
            ->segment(3));
        $this
            ->session
            ->set_userdata('print', $this
            ->uri
            ->segment(4));
    ...
    public function print_billing01()
    {
        ...
        $pm = $this
            ->db
            ->select('paciente,medico,centro_medico,id_rdv')
            ->where('idfacc', $idfacc)->get('factura2')
            ->row_array();
        $this->data['last_bill_id'] = $idfacc;
        $lastPatConDiag = $this
            ->db
            ->select('id_cdia, procedimiento, otros_diagnos')
            ->where('historial_id', $pm['paciente'])->where('id_user', $pm['medico'])->order_by('id_cdia', 'DESC')
            ->get('h_c_conclusion_diagno')
            ->row_array();
        $this->data['procedimiento'] = $lastPatConDiag['procedimiento'];
        $this->data['otros_diagnos'] = $lastPatConDiag['otros_diagnos'];
        $con_id_link = $lastPatConDiag['id_cdia'];
        $this->data['show_diagno_pat'] = $this
            ->model_admin
            ->show_diagno_pat($con_id_link);
        $this->data['billing1'] = $this
            ->model_admin
            ->showBilling1($idfacc);
        $this->data['billing2'] = $this
            ->model_admin
            ->showBilling2($idfacc);
        $html = $this
            ->load
            ->view('admin/print/billing', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_billing_report_date_range()
    {
        ...
        $html = $this
            ->load
            ->view('admin/print/print_billing_report_date_range', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    ...
    public function print_billing_seguro_privado_()
    {
        ...
        $this->data['show_diagno_pat'] = $this
            ->model_admin
            ->show_diagno_pat($pm['paciente'], $pm['medico'], $pm['centro_medico'], '');
        $this->data['procedimiento'] = $this
            ->db
            ->select('procedimiento')
            ->where('historial_id', $pm['paciente'])->where('id_user', $pm['medico'])->where('centro_medico', $pm['centro_medico'])->get('h_c_conclusion_diagno')
            ->row('procedimiento');
        $this->data['billing1'] = $this
            ->model_admin
            ->showBilling1($idfacc);
        ...
        $html = $this
            ->load
            ->view('admin/print/seguro-privado/billing_centro', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_patients_to_disabled()
    {
        ...
        $this
            ->db
            ->update("doctor_agenda_test", $updateData);
        $this->data['doc'] = $doc;
        $this->data['centro'] = $centro;
        $this->data['doctor'] = $this
            ->db
            ->select('name')
            ->where('id_user', $doc)->get('users')
            ->row('name');
        $this->data['centro_name'] = $this
            ->db
            ->select('name')
            ->where('id_m_c', $centro)->get('medical_centers')
            ->row('name');
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $html = $this
            ->load
            ->view('admin/print/print_patients_to_disabled', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function invoice_ars_p_v_()
    {
        $this
            ->session
            ->set_userdata('desde', $this
            ->uri
            ->segment(3));
        $this
            ->session
            ->set_userdata('hasta', $this
            ->uri
            ->segment(4));
        $this
            ->session
            ->set_userdata('id_ncf', $this
            ->uri
            ->segment(5));
        $this
            ->session
            ->set_userdata('is_admin', $this
            ->uri
            ->segment(6));
        $this
            ->session
            ->set_userdata('id_user', $this
            ->uri
            ->segment(7));
        $this
            ->session
            ->set_userdata('id_patient', $this
            ->uri
            ->segment(8));
    ...
    public function printinv001()
    {
        ...
        $invoice = $this
            ->model_admin
            ->print_ars_fac_found($this
            ->session
            ->userdata['id_ncf'], $id_patient);
        ...
        $this
            ->load
            ->view('medico/billing/invoice_ars_claim/after-save', $data);
    }
    public function print_ars_fac_found($id_ncfp, $id_centrop, $desde, $hasta, $id_patient)
    {
        $this
            ->session
            ->set_userdata('id_ncfp', $id_ncfp);
        $this
            ->session
            ->set_userdata('id_centrop', $id_centrop);
        $this
            ->session
            ->set_userdata('desdeinv', $desde);
        $this
            ->session
            ->set_userdata('hastainv', $hasta);
        $this
            ->session
            ->set_userdata('id_patient', $id_patient);
    ...
    public function print_ncf_report()
    {
        $this
            ->session
            ->set_userdata('medico_ncf', $this
            ->input
            ->get('medico-ncf'));
        $this
            ->session
            ->set_userdata('desde_ncf', $this
            ->input
            ->get('desde-ncf'));
        $this
            ->session
            ->set_userdata('hasta_ncf', $this
            ->input
            ->get('hasta-ncf'));
...
WHERE $condition && medico=$medico && value !='' group by value ORDER BY fecha DESC";
    ...
    public function printHist()
    {
        ...
        $this->data['print_enf_act'] = $this
            ->model_admin
            ->show_enfermedad($id_enf);
        $patient = $this
            ->db
            ->select('historial_id,centro_medico,inserted_time,user_id')
            ->where('id_enf', $id_enf)->get('h_c_sinopsis')
            ->row_array();
        $this->data['print_cond'] = $this
            ->model_admin
            ->show_con_by_id($id_cdia, 0);
        $this->data['procedimiento'] = $this
            ->db
            ->select('procedimiento')
            ->where('id_cdia', $id_cdia)->get('h_c_conclusion_diagno')
            ->row('procedimiento');
        ...
        $doctor = $this
            ->db
            ->select('name,area')
            ->where('id_user', $patient['user_id'])->get('users')
            ->row_array();
        ...
        $this->data['edad'] = $this
            ->db
            ->select('edad')
            ->where('id_p_a', $patient['historial_id'])->get('patients_appointments')
            ->row('edad');
        $this->data['ExamFisById'] = $this
            ->model_admin
            ->ExamFisById($id_sig);
        $this->data['signo_by_id'] = $this
            ->model_admin
            ->ExamFisById($id_sig);
        $this->data['show_oft'] = $this
            ->model_admin
            ->showOftalById($id_oftal);
        $this->data['print_recetas'] = $this
            ->model_admin
            ->print_recetas_email_patient($id_patm);
        $this->data['estudios'] = $this
            ->model_admin
            ->print_estudio($id_i_e);
        $this->data['IndicacionesLab'] = $this
            ->model_admin
            ->Printlaboratorios_email_patient($id_patl);
        $mpdf->setFooter("Página {PAGENO} de {nb}");
        $html = $this
            ->load
            ->view('admin/print/printHist', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function get_seguro_date_range()
    {
        ...
        $sqlgnl = "SELECT *,subtotal as t1, totalpaseg as t2, totpapat as t3 FROM factura2 JOIN factura ON factura.id2=factura2.idfacc WHERE CAST(filter_date AS DATE) BETWEEN '$desde' AND '$hasta' AND seguro_medico=$seguro AND $where GROUP BY numauto ORDER BY filter_date DESC";
        $this->data['querygnl'] = $this
            ->db
            ->query($sqlgnl);
        if ($centro_type == 'publico')
        {
            $html = $this
                ->load
                ->view('admin/print/patient-billing-report-by-date-centro', $this->data, true);
        }
        else
        {
            $html = $this
                ->load
                ->view('admin/print/patient-billing-report-by-date', $this->data, true);
        }
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_emg_f()
    {
        $this
            ->session
            ->set_userdata('id_mergencia', $this
            ->uri
            ->segment(3));
    ...
    public function print_emergency_fac()
    {
        ...
        $em_patient = $this
            ->db
            ->select('enviado_name,created_date,create_time')
            ->where('id_ep', $id_mergencia)->get('emergency_patients')
            ->row_array();
        ...
        $this->data['fecha_alta'] = $this
            ->db
            ->select('inserted_time')
            ->where('id_em', $id_mergencia)->get('emergency_conclucion_alta')
            ->row('inserted_time');
        $facingo = $this
            ->db
            ->select('*')
            ->where('id_mergencia', $id_mergencia)->get('emergency_factura')
            ->row_array();
        ...
        $centro = $this
            ->db
            ->select('name,logo,fax,primer_tel,segundo_tel,rnc,calle,barrio')
            ->where('id_m_c', $centro_id)->get('medical_centers')
            ->row_array();
        $this->data['centro_name'] = $centro['name'];
        $this->data['centro_logo'] = $centro['logo'];
        $this->data['calle'] = $centro['calle'];
        ...
        $seguro = $this
            ->db
            ->select('title,rnc')
            ->where('id_sm', $seguro_id)->get('seguro_medico')
            ->row_array();
        $this->data['seguro'] = $seguro['title'];
        $this->data['rnc'] = $seguro['rnc'];
        $this->data['seguro_codigo_contrato'] = $this
            ->db
            ->select('codigo')
            ->where('id_centro', $centro_id)->where('id_seguro', $seguro_id)->get('codigo_contrato')
            ->row('codigo');
        $paciente = $this
            ->db
            ->select('id_p_a,nombre,tel_resi,tel_cel,email,afiliado,cedula,photo,ced1,ced2,ced3,date_nacer')
            ->where('id_p_a', $patient_id)->get('patients_appointments')
            ->row_array();
        ...
        $this->data['numafiliado'] = $this
            ->db
            ->select('input_name')
            ->where('patient_id', $patient_id)->where('inputf', "No. DE AFILIADO")
            ->get('saveinput')
            ->row("input_name");
        ...
        $cie10 = $this
            ->db
            ->select('id,otros_diagnos')
            ->where('id_em', $id_emergencia)->get('emergency_conclucion_alta')
            ->row_array();
        $idcie10 = $cie10['id'];
        ...
        $html = $this
            ->load
            ->view('admin/print/emergency_factura', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function print_hosp1()
    {
        ...
        $this
            ->session
            ->set_userdata('id_seguro', $this
            ->uri
            ->segment(5));
        $this
            ->session
            ->set_userdata('id_doctor', $this
            ->uri
            ->segment(6));
        $this
            ->session
            ->set_userdata('id_centro', $this
            ->uri
            ->segment(7));
    ...
    public function print_hosp101()
    {
        ...
        $dato_ing = $this
            ->db
            ->select('causa,via,sala,servicio,cama,hosp_auto,hosp_auto_por,timeinserted,inserted,timeupdated,updated')
        ...
        $html = $this
            ->load
            ->view('admin/print/hospitalization-data', $this->data, true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
}
