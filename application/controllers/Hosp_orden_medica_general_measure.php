<?php
class Hosp_orden_medica_general_measure extends CI_Controller {
...
public function save_orden_medica_mg() {
    ...
    if( $this->input->post('ordenMedicaMedGen') == "" ){
        ...
        $response['status']  = 1;
           ...
           $this->hospitalization_emgergency->insert('hosp_ord_med_gen',$data);
          //$error = $this->hospitalization_emgergency->error();
               ...
               $this->hospitalization_emgergency->update('hosp_ord_med_gen',$data);
...
public function medidas_generales() {
   $id_register= $this->input->post('id_register');
    ...
    $data['perfil']= "Admin";
    $sql="SELECT * FROM  hosp_ord_med_gen WHERE id_register=$id_register  AND id_patient=$this->ID_PATIENT AND canceled=0 ORDER BY id  DESC";
    ...
    $this->load->view('hospitalization/clinical-history/medical-order/medida-general/table', $data);
...
{  
     $output = array();  
     $id=$this->input->post('id');
     $query=$this->hospitalization_emgergency->query("SELECT * FROM hosp_ord_med_gen WHERE id=$id");
...
{
    $id = $this->input->post('id');
  $this->hospitalization_emgergency->query("DELETE FROM hosp_ord_med_gen WHERE id=$id");
}
