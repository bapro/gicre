<?php
class Emerg_orden_medica_estudy extends CI_Controller {
    ...
    if( $this->input->post('ordenMedicaEstEst') == "" || $this->input->post('ordenMedicaEstBodyPart') == ""){
        ...
        $response['status']  = 1;
		 ...
		 if($estudio){
           ...
           $this->hospitalization_emgergency->insert('emerg_orden_medica_estudios',$data);
               ...
               $this->hospitalization_emgergency->update('emerg_orden_medica_estudios',$data);
...
public function studies() {
   $data['user_id']= $this->ID_USER;
    ...
    $sql="SELECT * FROM  emerg_orden_medica_estudios WHERE id_register=$ordenMedInsertedId AND historial_id=$this->ID_PATIENT AND canceled=0  ORDER BY id  DESC";
    ...
    $this->load->view('emergency/clinical-history/medical-order/study/table', $data);
...
{  
     $output = array();  
     $id=$this->input->post('id');
     $query=$this->hospitalization_emgergency->query("SELECT * FROM emerg_orden_medica_estudios WHERE id=$id");
...
{
    $id = $this->input->post('id');
  $this->hospitalization_emgergency->query("DELETE FROM emerg_orden_medica_estudios WHERE id=$id");
}
