...
class Hosp_notas extends CI_Controller
{
	...
	public function saveExamenFisico()
	{
		...
		if ($this->input->post('nombre_nota') == "" || $this->input->post('hallazgo') == "") {
			...
			if ($id > 0) {
				$this->db->where('id', $id);
				$this->db->update("hosp_signo_vitales", $save);
			} else {
				$this->db->insert("hosp_signo_vitales", $save);
			}
	...
	function saveExamenFisicoNotas()
	{
		...
		if ($this->input->post('idExF') == 0) {
			//INSERT NOTA
			$this->hospitalization_emgergency->insert('hosp_exam_fis_nota', $rowSigNota);
			...
			$this->hospitalization_emgergency->insert('hosp_signo_vitales', $rowSigVit);
			...
			$this->hospitalization_emgergency->insert('hosp_signos_vitales_results', $rowSigVitRslt);
		} else {
			$this->hospitalization_emgergency->where('id', $this->input->post('idExF'));
			$this->hospitalization_emgergency->update('hosp_exam_fis_nota', $rowSigNota);
			...
			$this->hospitalization_emgergency->update('hosp_signo_vitales', $rowSigVit);
			$row = array(
				'fr' => $this->input->post('signo_fr_result'),
				'fc' => $this->input->post('signo_fc_result'),
				'ft' => $this->input->post('signo_temp_result'),
			...
			$this->hospitalization_emgergency->update('hosp_signos_vitales_results', $row);
    ...
    public function form()
    {
        ...
        $sql = "SELECT *, hosp_exam_fis_nota.id AS idSearch FROM hosp_exam_fis_nota INNER JOIN  hosp_signo_vitales ON hosp_exam_fis_nota.id = hosp_signo_vitales.id_nota WHERE hosp_exam_fis_nota.id=$page";
        $query_ex_notas_fis = $this->hospitalization_emgergency->query($sql);
        $data['query_ex_notas_fis'] = $query_ex_notas_fis->result();

        $sqlSr = "SELECT * FROM hosp_signos_vitales_results WHERE id_sig=$page";
        ...
        $this->load->view('hospitalization/clinical-history/notas/form', $data);
    ...
    {
        $sql = "SELECT * FROM hosp_exam_fis_nota WHERE id_patient=$this->ID_PATIENT ORDER BY id DESC";
