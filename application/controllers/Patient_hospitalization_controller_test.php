...
class Patient_hospitalization_controller_test extends CI_Controller { 
public function __construct()
	{
		...
		$this->load->model('model_admin');
		 ...
		 $this->load->library("get_table_data_by_id");
		  $this->load->model('hospitalization_patients_data_model_test');
...
$query = $this->hospitalization_emgergency->query("SELECT * FROM hospitalization_data WHERE id_patient = $this->ID_PATIENT && canceled = 0 ORDER BY id DESC");
 ...
 function fetch_patient_data()
 {
  ...
  $output = array(
   ...
   'patient_data'   => $this->hospitalization_patients_data_model_test->fetch_patient_data($id_patient)
  );
...
$output = array(
  ...
  function fetch_patients_hoptalization_data()
 {
  ...
  $config = array();
  $config['base_url'] = '#';
  $config['total_rows'] = $this->hospitalization_patients_data_model_test->count_all($centro, $origine, $patient,$condition);
  $config['per_page'] = 10;
  ...
  $output = array(
   ...
   'product_list'   => $this->hospitalization_patients_data_model_test->fetch_data($config["per_page"], $start, $centro, $origine, $patient,$condition)
  );
  echo json_encode($output);
 }
