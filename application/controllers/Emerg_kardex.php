...
class Emerg_kardex extends CI_Controller {
...
{
$cantidad=$this->hospitalization_emgergency->select('new_cant')->where('id_i_m',$this->input->post('id'))->get('emerg_orden_medica_recetas')->row('new_cant'); 
echo $cantidad;	
...
function savePrintKardex(){
 ...
 $this->session->set_userdata('CURRENT_PRINT_KARDEX_TIME', $action_time);
        ...
        $this->hospitalization_emgergency->insert_batch('emerg_kardex_print', $arrayData);
...
$this->hospitalization_emgergency->update("emerg_orden_medica_recetas",$update);
...
$sql ="SELECT *  FROM  emerg_orden_medica_recetas WHERE historial_id=$this->ID_PATIENT && kardex=0 && centro=$this->ID_CENTRO order by id desc";
$query=$this->hospitalization_emgergency->query($sql);
$data['query_list_k'] =$query;
$data['nb_ex_neu'] =$query->num_rows();
$this->load->view('emergency/clinical-history/kardex/listado-med-kardex', $data);
}
...
$query=$this->hospitalization_emgergency->select('kardex')->where('kardex',0)->where('historial_id',$this->ID_PATIENT)->where('centro', $this->ID_CENTRO)->get('emerg_orden_medica_recetas');
...
$sql ="SELECT *  FROM  emerg_devolucion_almacen_lab
LEFT JOIN  emerg_orden_medica_recetas ON  emerg_devolucion_almacen_lab.id_i_m = emerg_orden_medica_recetas.id
WHERE $column order by emerg_devolucion_almacen_lab.id DESC";
$query=$this->hospitalization_emgergency->query($sql);
$data['nb_kardex_return'] =$query->num_rows();
$data['devo'] =$query;
$this->load->view('emergency/clinical-history/kardex/devolucion', $data);
}
...
$this->hospitalization_emgergency->insert("emerg_devolucion_almacen_lab",$save);
...
function kardex_list(){
	$sqlkx ="SELECT *  FROM  emerg_orden_medica_recetas WHERE historial_id=$this->ID_PATIENT && kardex=1 && centro=$this->ID_CENTRO order by insert_date DESC";
$querykx=$this->hospitalization_emgergency->query($sqlkx);
$data['querykardex'] =$querykx;
$data['nb_kardex'] =$querykx->num_rows();
$this->load->view('emergency/clinical-history/kardex/new-kardex', $data);
}
