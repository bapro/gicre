...
class Emerg_interconsultas extends CI_Controller {
...
{
   $sql="SELECT *, asked_time as inserted_time FROM emerg_solicutud_consulta WHERE id_patient=$this->ID_PATIENT ORDER BY id DESC";
...
$sql="SELECT * FROM emerg_solicutud_consulta WHERE id=$page";
...
$hospitalData=$this->db->select('causa')->where('id',$this->ID_HOSP)->get('emergency_data')->row_array();
...
$this->hospitalization_emgergency->insert("emerg_solicutud_consulta",$data);
}else{
	$where= array(
...
$this->hospitalization_emgergency->update('emerg_solicutud_consulta', $data);
}
...
$asked=$this->db->select('asked_by,asked_time,responded_time')->where('asked_by',$id_user)->where('id_patient',$patient_id)->get('emerg_solicutud_consulta')->row_array();
...
$asked=$this->db->select('asked_to,asked_time,responded_time')->where('asked_to',$id_user)->where('id_patient',$patient_id)->get('emerg_solicutud_consulta')->row_array();
...
$this->db->update('emerg_solicutud_consulta', $data);
...
$sql = "select * from emerg_solicutud_consulta where $contition id_patient=$id_patient  ORDER BY id desc limit $start,$per_page";
