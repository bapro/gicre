...
class Hosp_eval_con extends CI_Controller {
...
$query_evcond= $this->hospitalization_emgergency->query("SELECT * FROM hosp_eval_cond WHERE id=$page");
$data['query_evcond']=$query_evcond->result();

$this->load->view('hospitalization/clinical-history/evaluation-condition/form', $data);
echo "
<script>
$('.spinner-border').hide();
   $( '.form-select' ).select2( {
...
public function pagination() {
    $sql="SELECT * FROM hosp_eval_cond WHERE id_patient=$this->ID_PATIENT ORDER BY id DESC";
...
/*
$this->hospitalization_emgergency->insert("hosp_eval_cond",$saveeVon);
...
$result=$this->hospitalization_emgergency->update('hosp_eval_cond',$saveeVon);
}else{
$result=$this->hospitalization_emgergency->insert('hosp_eval_cond',$saveeVon);	
}
