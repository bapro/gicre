...
class Hosp_control_signos_vitales extends CI_Controller {
...
$timeupdate=$this->hospitalization_emgergency->select('inserted_time, id_user')->where('id',$id)->get('hosp_control_signos_vitales')->row_array();
...
if($id==0){
$this->hospitalization_emgergency->insert("hosp_control_signos_vitales",$data);
}else{
$where= array(
  'id' =>$this->input->post('id')
);
...
$this->hospitalization_emgergency->update("hosp_control_signos_vitales",$data);	
}
...
$this->load->view('hospitalization/clinical-history/control-signo-vitales/content',$data);
}
...
$data['user_id']=$this->ID_USER;
$sql = "select * from  hosp_control_signos_vitales where id_patient=$this->PATIENT_ID && centro=$this->CENTRO_ID ORDER BY id desc";
$data['query']= $this->hospitalization_emgergency->query($sql);
$this->load->view('hospitalization/clinical-history/control-signo-vitales/list',$data);
}
...
$data['id_sig_vit']=$id;
$this->load->view('hospitalization/clinical-history/control-signo-vitales/form', $data);
}
...
$this->hospitalization_emgergency->where($where);
$this->hospitalization_emgergency->delete('hosp_control_signos_vitales');

$sql = "select * from  hosp_control_signos_vitales where id_patient=$this->PATIENT_ID && centro=$this->CENTRO_ID";
$query= $this->hospitalization_emgergency->query($sql);
...
function savePrintCsv(){
 ...
 $this->session->set_userdata('CURRENT_PRINT_CSV_TIME', $action_time);
        ...
        $this->hospitalization_emgergency->insert_batch('hosp_csv_print', $arrayData);
