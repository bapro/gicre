...
$this->load->view('chat/chatHistorialData',$data);
	...
	$where = array(
	'user' =>$this->session->userdata['medico_id']
	);
	...
	$where1 = array(
	'userid' =>$this->session->userdata['medico_id'],
	'id_enfermedad' =>0
	);
...
$config['full_tag_open']    = "<ul class='pagination'>";
	 ...
	 if($this->session->userdata['medico_perfil']=="Medico"){
		 ...
		 $appointments= $this->model_medico->getAsMedicoApHoy($id_doctormd,$config["per_page"], $page); 
	...
	$plan= $this->db->select('plazo,cuenta_gratis,payment_plan')->where('id_user',$iduser)->get('users')->row_array();
	
	if($plan['plazo']=='0000-00-00'){
	 redirect("paypal/doctor_account_payment_/$iduser/1");
	}
	...
	if($plan['cuenta_gratis']==0){
	 ...
	 //doctor must pay 
		...
		//doctor must pay monthly
		if($plan['payment_plan']==1){
     $termDate=date('Y-m-d', strtotime($plan['plazo']. ' + 31 days'));
	if($current_date > $termDate) {
		 redirect("paypal/doctor_account_payment_/$iduser/1");
	}else {
		$this->load->view('medico/header',$data);
	...
	$termDate=date('Y-m-d', strtotime($plan['plazo']. ' + 365 days'));
	if($current_date > $termDate) {
	redirect("paypal/doctor_account_payment_/$iduser/1");
	}else {
		$this->load->view('medico/header',$data);
...
{
$message= $this->db->select('id_apoint')->where('confirmada',0)
->where('cancelar',0)
->where('doctor',$this->session->userdata['medico_id'])
->where('fecha_propuesta',date("d-m-Y"))
->where('seen_hoy',0)
->get('rendez_vous');
$data['result']=$message->num_rows();
...
public function historial_medical()
   ...
   $this->load->view('admin/historial-medical1/view-historial-clinica-past',$data);
	...
	//$this->load->view('medico/header',$data);
    //$this->load->view('medico/pacientes-citas/patients',$data);
...
//$data['area']=$this->session->userdata['id_esp'];
$data['is_admin']="no";
$data['number_cita']=count($query);
$this->load->view('medico/header',$data);
...
//$sql ="SELECT * FROM laboratory_lentes WHERE id_doc=$user_id && enviado=1 ORDER BY id DESC";
...
//$this->load->view('medico/billing/mssm/centro/create-centro',$data);
//}
}


//for asistente medico
...
//-------------------------------------------------------------------------------------------------
$seguro=$this->db->select('id_sm,title,logo')->where('id_sm',$seguro_id)
...
$codigo_contrato=$this->db->select('codigo,id,inserted_by,updated_by,updated_time,inserted_time')->where('id_seguro',$seguro_id)->where('id_doctor',$id_doc)
...
public function saveNewTarifMedico(){
	...
	'inserted_by'=>$this->session->userdata['medico_id'],
	'inserted_date'=>date("Y-m-d H:i:s"),
	'updated_by'=>$this->session->userdata['medico_id'],
...
//$this->load->view('medico/centers/medicos/medical_center', $data);
...
public function create_invoice_ars_claim()
{
	...
	$area_id=$this->db->select('area')->where('id_user',$id_user)
	->get('users')->row('area');
	$data['area_id'] = $area_id ;
	$data['area']=$this->db->select('title_area')->where('id_ar',$area_id)
...
$data['id_user'] =$id;
$this->load->view('medico/header',$data);
$this->load->view('medico/user/update', $data);
}
