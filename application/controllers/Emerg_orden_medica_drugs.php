<?php
class Emerg_orden_medica_drugs extends CI_Controller {
...
$this->load->view('emergency/auto-complete-field-results', $data);
      ...
      || $this->input->post('ordenMedFre') == "" || $this->input->post('ordenMedVia') == ""  || $this->input->post('ordenMedicaCantidad') == "" ){
        ...
        $response['status']  = 1;
		 ...
		 if($medicamento1){
           ...
           $this->hospitalization_emgergency->insert('emerg_orden_medica_recetas',$data);
               ...
               $this->hospitalization_emgergency->update('emerg_orden_medica_recetas',$data);
             
            }
			 [$num_drugs]= $this->model_emergency->count_total_each_table($this->input->post('idReceta'));
...
public function drugs() {
	...
	$ordenMedInsertedId=$this->input->post('ordenMedInsertedId');
	 $sql="SELECT * FROM  emerg_orden_medica_recetas WHERE id_register=$ordenMedInsertedId AND historial_id=$this->ID_PATIENT AND canceled=0 ORDER BY id DESC";
   ...
   $this->load->view('emergency/clinical-history/medical-order/drugs/table', $data);
...
{  
     ...
     $query=$this->hospitalization_emgergency->query("SELECT * FROM emerg_orden_medica_recetas WHERE id=$id");
