...
$('#createNewOrdenMedica').prop("disabled", true);	
 $.ajax({
type: "POST",
url: base_url_rep_hist+"emerg_orden_medica/create_orden_medica",
...
$(document).on('click', '.loadOrdMedMg', function(){ 
 ...
 function allLaboratorios(){
    ...
    $.ajax({
		type:'POST',
		url:base_url+ 'emerg_orden_medica_lab/patient_laboratories',
...
$(document).on('click', '.paginate-orden-medica-li', function() {
			...
			var controller = "emerg_orden_medica";
			var pageNum = this.id;
   ...
   event.preventDefault();
	resetFormOrdenMedica();
	
});


function resetFormOrdenMedica(){
...
$(document).on('click', '#hospOrdenMediSaveMed', function(event){ 
        event.preventDefault();
		 $('#show-alert-if-inputs-data input, #show-alert-if-inputs-data textarea, #show-alert-if-inputs-data select').removeClass('changed-input'); 
            ...
            $.ajax({
type: "POST",
url: base_url_rep_hist+"emerg_orden_medica_drugs/save_orden_medica_recetas",
data: {
...
$.ajax({
type: "POST",
url: base_url+"emerg_orden_medica_drugs/drugs",
...
if($("#searchLabOrdMedByName").val() !=""){
    $.ajax({  
                url: base_url+"emerg_orden_medica_lab/save_indication_lab",  
                method:"POST",  
...
$(document).on('click', '.orden-med-update-drug', function(){  
           ...
           $.ajax({  
                url: base_url+"emerg_orden_medica_drugs/fetch_single_drug",  
                method:"POST",  
                data:{id:id},  
                dataType:"json",  
                success:function(data)  
...
event.preventDefault();
 $('#show-alert-if-inputs-data input, #show-alert-if-inputs-data textarea, #show-alert-if-inputs-data select').removeClass('changed-input'); 
     ...
     $.ajax({
type: "POST",
url: base_url+"emerg_orden_medica_estudy/save_orden_medica_sutdy",
data: {
...
$.ajax({
type: "POST",
url: base_url+"emerg_orden_medica_estudy/studies",
...
event.preventDefault();
          ...
          $('#show-alert-if-inputs-data input, #show-alert-if-inputs-data textarea, #show-alert-if-inputs-data select').removeClass('changed-input');
           $.ajax({  
                url: base_url+"emerg_orden_medica_estudy/fetch_single_study",  
                method:"POST",  
                data:{id:id},  
                dataType:"json",  
                success:function(data)  
...
$('#show-alert-if-inputs-data input, #show-alert-if-inputs-data textarea, #show-alert-if-inputs-data select').removeClass('changed-input');
     ...
     $.ajax({
type: "POST",
url: base_url+ "emerg_orden_medica_general_measure/save_orden_medica_mg",
data: {
...
$.ajax({
type: "POST",
url: base_url+"emerg_orden_medica_general_measure/medidas_generales",
data: {id_register:$("#hospOrdenMedInsertedId").val()},
success: function(data){
...
$(document).on('click', '.orden-med-update-mg', function(){  
           ...
           $.ajax({  
                url: base_url+"emerg_orden_medica_general_measure/fetch_single_mg",  
                method:"POST",  
                data:{id:id},  
                dataType:"json",  
                success:function(data)  
...
var del_id = $(this).attr('id');
 ...
 var tableName='emerg_ord_med_gen';
 var field='canceled'; 
 var value = 1;
...
var el = this;
var del_id = $(this).attr('id');
 var tableName='emerg_orden_medica_recetas';
 var field='canceled'; 
 var value = 1;
...
var del_id = $(this).attr('id');
 ...
 var tableName='emerg_orden_medcia_lab';
...
var del_id = $(this).attr('id');
 ...
 var tableName='emerg_orden_medica_estudios';
 var field='canceled'; 
 var value = 1;
saveReasonToCancelTable(el, del_id, reasonToCancelTable, tableName,field, value);
loadOrdenMedStudy();
...
window.open(base_url +'print_page_emergency/medical_order/'+ idDocPrint + "/" +idPatientPrint + "/" +idOrden, '_blank', 'noopener, noreferrer');
 ...
 $(document).on('click','#repeat-orden-medica',function(event){
	...
	$.ajax({
	type: "POST",
	url:base_url+"emerg_orden_medica/repetirOrdenMedica",
